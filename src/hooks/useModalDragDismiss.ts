    ...
    const onEnd = () => {
      if (mode === "drag" && sheet) {
        ...
        if (sheetShift >= CLOSE_THRESHOLD || flicked) {
          ...
          // 목표 거리는 "자기 높이의 %"가 아니라 화면 기준으로 — 예전 120%(자기 높이의
          // 1.2배)는 키 큰 모달(랭킹 상세 등 88vh)에선 화면 밖까지 충분했지만, 키 작은
          // 모달(회원 프로필 등)에선 화면 중간까지밖에 못 내려가 슬라이드가 중간에 멈춘 채
          // 언마운트돼 "뚝 멈추고 갑자기 사라지는" 것처럼 보였다(지적). 시트류는 모두 화면
          // 세로 중앙 배치라, 중앙(50vh)에서 자기 높이 절반(50%)+여유(10%)만큼 내리면
          // 높이와 무관하게 항상 아래 화면 밖으로 완전히 빠져나간다.
          s.style.translate = "0 calc(50vh + 60%)";
