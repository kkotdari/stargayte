...
/* 급마다 달리 두던 기본 비율을 5%로 합친다(요청: 하한선 내린 만큼 조건 수치 상향) —
   ...
   비율만 남은 지금 그 자리를 대신한다. 5%(마흔 판에 두 번)에서 7%로 한 번 더 올렸다(요청)
   1급·2급을 가르지 않는 것은 드묾을 이미 무게(TIER_BOOST)가 값으로 쳐 주기 때문이다. */
const COUNT_SHARE: Record<number, number> = { 1: 0.07, 2: 0.07 };
...
/* 낮은 비율들을 한 차례 올렸다(요청) — 비례 최소 횟수(countMinFor)를 걷으면서 잣대가
   ...
   올림(40×0.01)=1). 드문 수라도 한 번은 우연이라, 마흔 판에 세 번은 나와야 하는 선
   (7%)으로 맞춘다. 백 판이면 일곱 번이다 — 두 번은 아직 우연이 섞이는 수이고, 세 번쯤
...
const TITLES: Title[] = [
  ...
  { ...tactic("조이기 퀸", ["center-tank", "side-tank"]), minPlaysShare: 0.07 },
  ...
  { ...tactic("메카닉 사령관", ["mech"]), minPlaysShare: 0.07 },
  ...
  { ...tactic("목동저그", ["moka"]), minPlaysShare: 0.07 },
  ...
  { ...rare("개통의 달인", ["nydus"]), minPlaysShare: 0.07 },
  { ...rare("리콜 배달부", ["recall"]), minPlaysShare: 0.07 },
  ...
  { ...tactic("몰래배럭 퀸", ["sneak-rax"]), minPlaysShare: 0.07 },
  ...
  { ...rare("다크스웜 살포반", ["swarm"]), minPlaysShare: 0.07 },
