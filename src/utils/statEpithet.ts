...
export interface EpithetSubject {
  ...
  /** 이번 달에 레이팅이 얼마나 올랐나 — "떠오르는 샛별"이 쓰는 값(요청: 최근 급상승).
   *
   *  레이팅은 '그 날짜까지의 기록으로 본 값'이라, 이번 달 것과 지난달 것의 차이가 곧 그 달에
   *  얼마나 올랐나다(통계 표의 레이팅 변동과 같은 계산). 두 달 중 한쪽이라도 순위 대상이
   *  아니면 없다 — 견줄 자리가 없는 값으로 '급상승'을 말할 수는 없다. */
  rise?: number;
  /** 이 사람이 등록한 경기 수 — "기록 퀸"이 쓰는 값(요청). 뛴 것이 아니라 올린 것이다. */
  registered?: number;
}
...
const TITLES: Title[] = [
  ...
  /* 클럽의 기록을 남기는 사람(요청: 게임결과 등록 많은 것 → 기록 퀸) — 남의 경기까지 대신
     올려 준 사람에게도 쌓이는 값이다. 판을 뛴 것과는 아예 다른 종류의 공이라 무게도 높다. */
  { label: "기록 퀸", weight: 4, why: "등록한 경기", unit: "건", value: (_s, of) => of.registered ?? null },

  /* 개인전을 많이 치른 사람(요청: 도장깨기 전문가) — 팀전은 넷이 함께 한 판이지만 개인전은
     제 이름으로 붙는 판이라, 그 수가 곧 '얼마나 자주 붙었나'다. */
  { label: "도장깨기 전문가", weight: 3, why: "개인전 판수", unit: "판", value: (_s, of) => of.solo?.plays ?? null },

  /* 최근 급상승(요청: 떠오르는 샛별) — 이번 달에 레이팅이 가장 많이 오른 사람.
     이 표에서 유일하게 '지금 어떤가'가 아니라 '얼마나 달라졌나'를 재는 칭호다. 그래서 문턱도
     크다: 20점은 그 달에 확실히 치고 올라온 사람의 폭이고, 그 아래는 오르내림에 가깝다. */
  { label: "떠오르는 샛별", weight: 4, min: 20, why: "이번 달 레이팅 상승", unit: "", value: (_s, of) => of.rise ?? null },
