...
// 0.3 → 0.2(요청: 게임수 상수 20) — 월 40판 클럽에서 기준 8판.
const MONTHLY_FLOOR_RATE = 0.2;
...
/* (삭제·지적) 카운트 하한(countMinFor·CLUB_COUNT_RATE·COUNT_MIN_FLOOR/CAP) — 비율 하한
   (minPlaysShare)·표본 바닥(ratioFloor)과 별개로 "최소 2~6번(클럽 판수 비례)"을 또 걸던
   셋째 문턱이다. 설명 표에 세 잣대가 나란히 찍히니 겹말이 됐고(지적: 최소 2~6번이랑 5판부터
   중복이고 안 맞는다), 실제로도 표본 바닥과 비율이 이미 같은 우연을 막는다 — 잣대를
   "비율 + 표본" 둘로 줄인다. */
...
/* (삭제·요청) combatRate — 지상전/공중전/마법 퀸이 쓰던 전투 승률. 전투 원장 자체
   (combat·bt_*)는 서버의 에픽 전투 게이트가 계속 쓴다. */
...
/* (삭제·지적) KIND_RANK — 갈래(한방·승률·명예·경기력)로 동점대를 묶던 표. 대표 선정의
   묶음이 설명 표의 손차례(GUIDE_ORDER)로 넘어가며 걷었다 — 두 화면이 서로 다른 차례를
   말하면 안 된다. kind 필드 자체는 이름표로 남는다. */
...
const TITLES: Title[] = [
  ...
  /* 5 → 50%(요청: 다른 칭호와 비슷한 희귀도로) — 판당 사용까지 세는 값(perUse)이라 핵으로
     사는 사람은 수백 %가 나온다. 5%는 몇 번 쏴 본 사람까지 다 받는 문턱이었다. */
  { ...spell("정은 퀸", "Nuclear Strike"), minPlaysShare: 0.5 },
  ...
  /* (삭제·요청) 얼음 공주 — 마엘스트롬·스테이시스·락다운 합산. */
  // (삭제·요청) 거미줄 설계자(디스럽션 웹).
  ...
  /* 끈질긴 견제(harass-long)도 함께 센다(지적: 일꾼 헌터가 너무 안 나온다) — 자막이
  ...
  // (삭제·요청) 여전사 발키리(valkyrie) — 오버로드 사냥꾼(valk-hunt)이 그 유닛의 이야기를 맡는다.
  ...
  /* 함대의 여왕(요청 — 여신은 전설급에만 쓴다) — 아비터까지 함께 굴렸으면 "그림자 함대의
     ...
     수로 본다. */
  /* 날파리 퀸은 캐리어만 본다(요청) — 아비터 조합 갈래(보이지 않는 날파리 퀸)는 걷었다:
     아비터의 이야기는 시공을 지배하는 여제(arbiter)가 따로 맡는다. */
  { ...tactic("캐리어를 모으는 퀸", ["carrier"]), minPlaysShare: 0.05, battle: true },
  ...
  // (삭제·요청) 보이지 않는 여왕(dark-templar) — 다크 운용 칭호.
  ...
  { ...tactic("오버로드 사냥꾼", ["valk-hunt"]), race: undefined, minPlaysShare: 0.05 },
  ...
  // (삭제·요청) 부지런한 정찰 퀸(vision) — 초반 정찰. 전장을 살피는 눈만 남는다.
  { ...tactic("전장을 살피는 눈", ["vision-eye"]), minPlaysShare: 0.8 },
  ...
  // (삭제·요청) 다크스웜 살포반 — 스웜의 이야기는 목동의 여왕이 맡는다.
  ...
  { ...tactic("철벽 퀸", ["hold-off"]), minPlaysShare: 0.15 },
  ...
  { ...tactic("불사조", ["lodging", "relocate"]), why: "본진 밖 생존(이사·셋방살이)", minPlaysShare: 0.2, weight: 3 },
  ...
  // (삭제·요청) 군더더기 없는 손놀림 — APM 중 유효타 비율.
     ...
     말인데, 채우는 것이 "제일 먼저 죽었다"면 비워 두는 편이 낫다. */
  {
    ...
    /* 올라운드 강자(요청: 개명 + 전설급으로 — 종족퀸 아래) — 종족은 셋뿐이라 "최소 3개"는
       곧 전부다: 설명도 "모든 종족 승률 50% 이상"으로 적는다(특례). */
    label: "올라운드 강자", weight: 3, kind: "경기력", sticky: true, pool: 1, edge: 1, min: 3, scale: "count",
  ...
  // 전설급으로(요청) — 종족퀸 아래 자리다(차례는 GUIDE_ORDER가 정한다).
  { label: "BEST PLAYER", weight: 6, kind: "명예", sticky: true, min: 0.35, why: "판 대비 BEST 선정", unit: "",
...
export interface EpithetGuideRow {
  ...
  how: string;
  rank: EpithetRank;
  /** 목록의 묶음 번호(요청: 그룹화) — 모달이 묶음이 바뀌는 자리에 줄을 긋는다. */
  group: number;
  /* (삭제·요청) minPlays(최소판 열) — 표본 바닥은 클럽 판수 비례라 값이 늘 움직이고,
     조건을 읽는 눈에 곁가지라 열째로 걷었다. 바닥 자체(ratioFloor 등)는 그대로 돈다. */
}

/* 설명 목록의 차례와 묶음(요청: 그룹화를 다시 해봤다 — 손으로 짠 차례 그대로).
   묶음의 결: [종합·명예] [전술·전투 조합] [경기 자세]. 여기 없는 이름은 맨 뒤 묶음으로
   떨어진다 — 새 칭호를 넣고 이 표를 잊어도 목록에서 사라지지는 않게. */
const GUIDE_ORDER: string[][] = [
  ["승리의 여신"],
  ["저그의 절대군주 · 프로토스의 전설 · 테란의 영웅"],
  // 전설급으로 올렸다(요청) — 종족퀸 아래 자리.
  ["BEST PLAYER", "올라운드 강자"],
  ["개근왕", "○○의 여왕"],
  [
    ...
    "캐리어를 모으는 퀸", "또각또각 번개 퀸",
    "리버 드랍의 여왕", "시공을 지배하는 여제", "깜짝 번개 퀸",
    "가시의 여왕", "뮤탈 습격 퀸", "초반의 여왕", "꽃게탕 퀸",
    "아군 보호 퀸", "커널 개통 퀸", "오버로드 사냥꾼",
    "리콜 배달 퀸",
  ],
  [
    // 경기 자세 묶음 — 차례는 붙여 준 표 그대로(요청: 그루핑 다시).
    "풀업 퀸", "심시티 퀸", "경제력 퀸", "헬프 퀸", "역전의 명수", "전장을 살피는 눈", "철벽 퀸", "불사조",
  ],
];
/** {n} 칭호(맵·종족·캐리어)는 사람마다 라벨이 달라 표의 이름과 못 맞춘다 — 표에 적힌
...
const guidePlaceOf = (label: string): { group: number; order: number } => {
  for (let g = 0; g < GUIDE_ORDER.length; g += 1) {
    const i = GUIDE_ORDER[g].indexOf(label);
    if (i >= 0) return { group: g, order: i };
  }
  return { group: GUIDE_ORDER.length, order: 0 };
};
...
export function epithetGuideRows(): EpithetGuideRow[] {
  ...
  const rows = TITLES.map((t) => {
    ...
    if (count) {
      ...
      /* "이긴 판만" 꼬리 대신 문장에 승리를 붙인다(요청: "역전 승리" 이렇게). 전투 칭호는
         제 꼬리("전투/게임 모두 승리")가 그 말을 하므로 안 붙인다. */
      const winWord = t.won === true && t.battle !== true ? " 승리" : "";
      if (share > 0 && t.perUse) bits.push(`${what}${winWord} 게임 수 대비 ${pct(share)} 이상`);
      else if (share > 0) bits.push(`${pct(share)} 이상 게임에서 ${what}${winWord}`);
      else bits.push(`${what}${winWord}`);
  ...
  return rows.map((r) => {
    ...
    if (r.label === "승리의 여신") {
      return { ...r, how: "승률 65% 이상" };
    }
    /* 올라운드 강자 — 자동 문구("5할 넘긴 종족 · 최소 3개")는 셈의 말이라, 뜻 그대로
       적는다(요청): 종족은 셋뿐이니 셋 다면 곧 전부다. */
    if (r.label === "올라운드 강자") {
      return { ...r, how: "모든 종족 승률 50% 이상" };
    }
    ...
    if (r.how.startsWith("그 맵 승수")) {
      return {
        ...r,
        label: "○○의 여왕",
        how: `그 맵 승률 ${Math.round(MAP_MIN_RATE * 100)}% 이상`,
      };
    }
    return {
      ...r,
    ...
    /* 차례는 손으로 짠 묶음표가 정한다(요청: 그룹화) — 등급이 먼저인 것은 그대로다(전설
       묶음 둘이 표의 맨 앞이라 결과는 같지만, 표를 어긋나게 고쳐도 등급이 섞이지 않게). */
    .sort((a, b) => (Number(b.rank === "전설") - Number(a.rank === "전설"))
      || (a.group - b.group) || (a.order - b.order) || (b.score - a.score))
    .map(({ label, how, rank, group }) => ({ label, how, rank, group }));
}
  ...
  TITLES.forEach((title, order) => {
    ...
    for (const p of ranked) {
      ...
      /* 대표를 고르는 세 겹(요청: 비슷비슷한 것들은 동점대로 묶고 횟수·퍼센트가 높은 걸
         ...
         ② 동점대(band): 같은 등급 안에서는 손으로 짠 묶음표(GUIDE_ORDER)의 앞 묶음이
            먼저다(지적: 아까 정한 그룹이 우선순위) — [종합·명예] › [전술·전투] › [자세].
      ...
      claims.push({
        ...
        /* 동점대 — 손으로 짠 묶음표(GUIDE_ORDER) 그대로다(지적: 아까 정한 그룹이 곧
           우선순위). 앞 묶음이 위다: [종합·명예] › [전술·전투 조합] › [경기 자세].
           한때 갈래(KIND_RANK)로 묶었는데, 그건 표와 화면이 서로 다른 차례를 말하는
           길이었다 — 설명이 보여주는 묶음과 대표를 고르는 묶음은 한 벌이어야 한다.
           음수인 것은 정렬이 "클수록 위"(b.band - a.band)라서다. */
        band: -guidePlaceOf(guideLabelOf(title)).group,
  ...
  const whyOf = (c: Claim): string => {
    ...
    if (countable) {
      /* 무리는 문턱이 잰 그 무리다(지적: 퍼센트가 아니라 수로만 나오는 줄이 있다) — 한때
         ...
         표본 바닥 위라 0일 수가 없다. */
      /* 분모는 기준 판수와 실제 판수 중 큰 쪽이다(지적: "역전 31판 (기준 155%)" — 기준
         판수만 분모로 쓰면 그 수를 기준보다 자주 낸 사람의 %가 100을 넘어 수가 이상해
         보인다). 문턱은 기준으로 재지만(그래서 넘치는 쪽은 어차피 통과), 근거는 읽는
         자리라 실제보다 큰 %를 말하면 안 된다. */
      const actual = c.title.vsWins ? c.wonDenom : c.denom;
      const pool = Math.max(normDenom(), actual);
      const poolName = actual > normDenom() ? "게임" : "기준";
