...
const COUNT_SHARE: Record<number, number> = { 1: 0.1, 2: 0.15 };
...
const COUNT_MIN: Record<number, number> = { 1: 3, 2: 4 };
/* 한때 상한을 뒀다("아무리 많이 뛰어도 여섯 번이면 인정") — 걷어냈다(지적: 상한보다 비율
   ...
   절대평가로 바꾸며 크게 올렸다가(1급 10%·2급 20%) 2급만 15%로 되물렸다(시뮬레이션 실측) —
   20%에서는 옆탱·조이기·드랍 같은 전술 칭호가 통째로 잠기고, 정작 흔한 대목(격차·정찰·역전)
   만 남았다. 그 흔한 대목들은 이제 제 문턱을 따로 가지므로(위 minPlaysShare 오버라이드)
   기본값은 전술이 닿을 수 있는 선이면 된다 — 마흔 판이면 여섯 번, 드문 수는 네 번이다. */
...
const TITLES: Title[] = [
  ...
  { ...spell("스톰 술사", "Psionic Storm"), minPlaysShare: 0.55 },
  ...
  { ...tactic("부지런한 정찰 퀸", ["vision"]), minPlaysShare: 0.45 },
     ...
     아닌가로 갈라 두 칭호를 두면 표에서는 결국 같은 사람이 두 번 걸린다. */
  {
    /* 판당으로 잰다(요청: 절대평가) — 누적 채수는 오래 뛴 사람이 늘 크다. 서른 채면 한 판에
       확장·생산·방어를 고루 편 살림이다. */
    label: "문어발 건축 퀸", weight: 3, min: 55, why: "판당 지은 건물", unit: "채",
    value: (s, of) => {
      const m = mix(s, of);
  ...
  { ...tactic("부티 자원 퀸", ["worker-gap"]), minPlaysShare: 0.65 },
  /* 흔한 대목들의 제 문턱(시뮬레이션 실측: 격차·정찰·역전·GG·스톰은 기본 문턱으로는 거의
     전원이 걸렸다 — 자막이 그 대목을 워낙 자주 짚는다) — 기본값 대신 그 대목의 실제 빈도에
     맞는 비율을 따로 건다. 시뮬레이션을 보며 세 차례 올렸다(지적: 아직도 너무 잘 나온다) —
     격차 65%·스톰 55%·정찰 45%·매너/역전 40%·헬프 35%. 자막이 그 대목을 짚는 빈도 자체가
     높아서, "그 사람의 색"이 되려면 이긴 판의 절반 안팎에서는 나와야 한다. */
  { ...tactic("다산의 상징", ["prod-gap"]), minPlaysShare: 0.65 },
  ...
  { ...tactic("매너 퀸", ["gg"]), minPlaysShare: 0.4 },
     ...
     새 재료가 필요 없고, 그런데도 "그 사람이 게임을 어떻게 다루나"를 말해 준다. */
  {
    ...
    /* 2.5 → 3.5(요청: 퀸으로 올릴 만하다) — 손 이야기 셋(유효타·APM·커맨드) 가운데 가장
       ...
       정해 두어야 나온다. */
    label: "군더더기 없는 손", weight: 3.5, min: 0.93, why: "APM 중 유효타", unit: "",
  ...
  /* 절대 문턱들(요청: 상대가 아니라 절대평가) — "클럽 1위"가 아니라 "이 수를 넘으면 그
     ...
     그 언저리에 선을 긋는다. */
  { label: "물량 퀸", weight: 3.5, min: 15, why: "분당 뽑은 기수", unit: "기", value: (s, of) => { const m = mix(s, of); return m ? perMin(m.coreUnit, won(s, of).mixSeconds) : null; } },
