...
/* 급마다 달리 두던 기본 비율을 5%로 합친다(요청: 하한선 내린 만큼 조건 수치 상향) —
   ...
   비율만 남은 지금 5%로 맞추면 옛 잣대가 그대로 선다(마흔 판에 두 번 · 백 판에 다섯 번).
   1급·2급을 가르지 않는 것은 드묾을 이미 무게(TIER_BOOST)가 값으로 쳐 주기 때문이다. */
const COUNT_SHARE: Record<number, number> = { 1: 0.05, 2: 0.05 };
...
/* 낮은 비율들을 한 차례 올렸다(요청) — 비례 최소 횟수(countMinFor)를 걷으면서 잣대가
   ...
   올림(40×0.01)=1). 드문 수라도 한 번은 우연이라, 마흔 판에 두 번은 나와야 하는 선
   (3~4%)으로 맞춘다. 백 판이면 서너 번이다. */
...
const TITLES: Title[] = [
  ...
  /* 조이기 퀸(요청: 개명 · 탱크 센터와 옆탱 포함) — 센터에 탱크를 박아 길목을 잠그는 것과
     아군·제 기지 옆을 탱크로 잡는 것은 같은 수의 두 자리다. 따로 두었을 때는 옆탱 쪽이 너무
     넓어 무엇을 잘한 사람인지가 안 잡혔는데(그래서 옆탱 칭호는 걷었다), 하나로 묶으면
     "탱크로 자리를 잠그는 사람"이라는 한 말이 된다. 문턱도 함께 내린다(3 → 2%). */
  { ...tactic("조이기 퀸", ["center-tank", "side-tank"]), minPlaysShare: 0.05 },
  ...
  { ...tactic("집요한 일꾼 헌터", ["harass-workers"], 1), minPlaysShare: 0.05 },
  ...
  { ...tactic("목동저그", ["moka"]), minPlaysShare: 0.05 },
  ...
  { ...rare("개통의 달인", ["nydus"]), minPlaysShare: 0.05 },
  ...
  { ...tactic("몰래배럭 퀸", ["sneak-rax"]), minPlaysShare: 0.05 },
  ...
  { ...rare("다크스웜 살포반", ["swarm"]), minPlaysShare: 0.05 },
  ...
  { ...tactic("역전 퀸", ["revival"]), minPlaysShare: 0.6 },
  ...
  {
    /* 생존퀸(요청: 끝까지 안 죽은 판) — 값은 '무너지지 않은 판의 비율'이다.
       세는 방법이 뒤집혀 있는 까닭: 요약은 '끝까지 살아 있었다'를 따로 적지 않는다(그건
       장면이 아니다). 대신 무너진 사람은 fallen으로 적히므로, 판수에서 그것을 빼면 남는
       것이 곧 버틴 판이다. fallen은 이긴 판만 세는 다른 전술과 달리 진 판에도 적힌다 —
       무너지는 장면이 원래 그쪽에서 나오기 때문이다.
       열 판 남짓으로는 비율이 안 선다(한 번 무너지면 10%가 깎인다). 열다섯 판부터 본다.
       주의: fallen은 '일찍 끊긴 사람'만 짚는다(replaySummary의 earlyOuts) — 끝까지 붙어
       있다가 진 판은 여기 안 걸린다. 그래서 이 값은 '안 죽었다'가 아니라 '무너진 적이
       거의 없다'에 가깝고, 근거 문장도 그렇게 적는다. */
    /* 갈래를 수비에서 경기력으로 올린다(요청: 퀸이 들어간 칭호가 너무 아래 갈래면 위로) —
       "퀸"이라 부르는 자리가 대표로는 늘 뒤로 밀리면 그 이름이 무색해진다. 웅크림을 세는
       칸(수비)에는 철옹성의 여인이 남는다. */
    label: "생존 퀸", weight: 2, kind: "경기력",
  ...
  { ...tactic("노엘을 외치는 여인", ["no-elim"]), minPlaysShare: 0.15 },
