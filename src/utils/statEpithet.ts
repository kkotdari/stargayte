...
const TITLES: Title[] = [
  ...
  /* (삭제) 브루들링 저격수 · 눈을 멀게 하는 자(옵티컬 플레어) · 허깨비 부대장(할루시네이션)
     ...
     아니라 이름만 요란해진다. */
  { ...tactic("헬프 퀸", ["ally-help"]), minPlaysShare: 0.5 },
  ...
  /* 돈 불리기 퀸(요청: 자원 제일 많이 모은 사람) — 자막의 일꾼 격차(worker-gap) 대목이다:
     상대보다 일꾼을 훨씬 많이 굴렸다는 것이 곧 자원을 제일 많이 캤다는 말이다. 흔한
     대목이라(격차류는 자막이 자주 짚는다) 문턱은 절반이다. 한때 "부티 자원 퀸"으로 있다
     걷힌 자리의 부활이다. */
  { ...tactic("돈 불리기 퀸", ["worker-gap"]), minPlaysShare: 0.5 },
...
const GUIDE_ORDER: string[][] = [
  ...
  [
    ...
    "풀업 퀸", "심시티 퀸", "돈 불리기 퀸", "헬프 퀸", "역전의 명수", "전장을 살피는 눈", "철벽 퀸", "불사조",
  ],
];
