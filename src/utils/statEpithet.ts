...
const TITLES: Title[] = [
     ...
     웹)과 달리 프로토스면 누구나 쓰는 수라 격을 더 올릴 자리는 아니다. */
  {
    ...
    label: "스톰 술사", weight: 2, race: "프로토스", min: 5, why: "판당 스톰", unit: "회",
    value: (s, of) => {
     ...
     아닌가로 갈라 두 칭호를 두면 표에서는 결국 같은 사람이 두 번 걸린다. */
  {
    /* 판당으로 잰다(요청: 절대평가) — 누적 채수는 오래 뛴 사람이 늘 크다. 서른 채면 한 판에
       확장·생산·방어를 고루 편 살림이다. */
    label: "문어발 건축 퀸", weight: 2, min: 70, why: "판당 지은 건물", unit: "채",
    value: (s, of) => {
      const m = mix(s, of);
  ...
  /* 무게를 2 → 1.2로 내렸다(요청) — 초반 일꾼은 그 판의 빌드가 정하는 값에 가깝다.
     ...
     다른 칭호들보다 작다. */
  { label: "일꾼 공장장", weight: 2, min: 40, why: "초반 5분 일꾼", unit: "기", value: (s, of) => won(s, of).avgWorker5 },
