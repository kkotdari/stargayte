...
const TACTIC_WEIGHT: Record<string, number> = {
  ...
  revival: 6,
  // 올인(도박 퀸) — 뒤를 안 남기고 건 판이 통한 것이라 퀸급이다(요청). 4 → 5(요청: 나오기
  // 힘든 순서) — 통했을 때만 세므로, 걸었다는 사실이 아니라 걸어서 이겼다는 뜻이다.
  allin: 5,
  ...
  "harass-workers": 5, "harass-long": 5, dropship: 5, "base-raid": 3,
...
const TITLES: Title[] = [
  ...
  { label: "물량퀸", weight: 3.5, why: "분당 뽑은 기수", unit: "기", value: (s, of) => { const m = mix(s, of); return m ? perMin(m.coreUnit, won(s, of).mixSeconds) : null; } },
  ...
  { label: "졌잘싸", weight: 3, why: "진 판의 BEST PLAYER", unit: "회", value: (s) => ((s.lostBests ?? 0) > 0 ? s.lostBests! : null) },
