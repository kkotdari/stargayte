...
/* 이름 짓는 규칙(요청) — 점수대가 곧 호칭의 격이다.
   격은 낱말로 나눈다(요청: 여신 › 퀸 › 그 외).
     ...
     퀸    : 3점 이상 — 그 판을 끌고 간 수(러시·운영·맵·헬프…). 이름은 "퀸" 아니면
       "○○의 여왕"으로 끝난다(요청) — 갈래와 이름이 같은 말을 해야 목록에서 격이 읽힌다.
       예외는 말장난 하나로 완성된 이름(퀸정은)뿐이다.
   ...
   흐려졌다. 딱 하나 "얼음 공주"만 남긴다(요청: 얼음 공주는 예외) — 마엘스트롬으로 얼려
   놓는 그림과 그 말이 붙어 있어, 격이 아니라 이름으로 읽힌다.
...
const TITLES: Title[] = [
     ...
     뜻이라, 그 자체가 그 사람의 취향이다. 값은 셋을 합친 수 — 많이 굴린 쪽이 임자다. */
  {
    ...
    label: "은신 퀸", weight: 2.5, pool: 1, edge: 1, tier: 1,
  ...
  tactic("초반러시 퀸", ["zealot-rush", "zling-rush"]),
     ...
     아닌가로 갈라 두 칭호를 두면 표에서는 결국 같은 사람이 두 번 걸린다. */
  {
    ...
    label: "건축 퀸", weight: 3, why: "지은 건물", unit: "채",
    value: (s, of) => {
     ...
     새 재료가 필요 없고, 그런데도 "그 사람이 게임을 어떻게 다루나"를 말해 준다. */
  {
    ...
    /* 2.5 → 3.5(요청: 퀸으로 올릴 만하다) — 손 이야기 셋(유효타·APM·커맨드) 가운데 가장
       ...
       정해 두어야 나온다. */
    label: "유효타 퀸", weight: 3.5, min: 0.72, why: "APM 중 유효타", unit: "",
  ...
  { label: "손놀림 퀸", weight: 3, why: "APM", unit: "", value: (s) => s.avgApm },
  ...
  { label: "커맨드 퀸", weight: 3, why: "분당 커맨드", unit: "", value: (s) => s.avgCmd },
...
export type EpithetRank = "여신" | "퀸" | "일반";
