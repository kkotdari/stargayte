...
const TITLES: Title[] = [
  ...
  /* 코끼리떼를 모는 여인(요청) — 울트라리스크를 모아 나간 판이다. 저그의 마지막 지상
     병력이라 거기까지 판을 끌고 갔다는 말이기도 한데, 부르는 말은 그 그림 하나면 된다.
     '조련사'에서 바꿨다(요청) — 한 마리를 길들이는 그림이 아니라 떼로 몰고 나가는 그림이고,
     '○○를 모으는 여인'(캐리어·배틀크루저)과 같은 꼴이라 유닛 칭호로 한눈에 읽힌다.
     자막이 그 대목을 'ultra'로 짚는다(replaySummary의 PROD_ONLY_KEYS와 같은 열쇠).
     비율은 다른 유닛 칭호와 같은 자리(UNIT_TACTIC_SHARE)에서 받는다. */
  tactic("코끼리떼를 모는 여인", ["ultra"]),
  ...
  { label: "BEST 퀸", weight: 6, kind: "명예", min: 0.35, why: "판 대비 BEST 선정", unit: "",
