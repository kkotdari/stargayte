...
const TITLES: Title[] = [
  ...
  { ...tactic("역전의 명수", ["revival"]), minPlaysShare: 0.32 },
  ...
  /* 흔한 대목들의 제 문턱(시뮬레이션 실측: 격차·정찰·역전·GG·스톰은 기본 문턱으로는 거의
     ...
     정찰 35% · 역전 32% · 매너 28% · 헬프 20%. 실측이 바뀌면 이 값도 같이 움직여야 한다. */
  ...
  /* 졌잘싸는 명예류에서 뺀다(지적: 베스트와 같이 묶이면 안 된다) — 진 판의 BEST라 잘 싸운
     것은 맞지만, 이긴 판을 만든 공로(BEST·참여)와 같은 동점대에 서면 대표 다툼에서 그 둘을
     성취도로 이길 수 있다(4회 문턱이 낮아 배수가 크게 나온다). 전술과 같은 4다. */
  { label: "졌잘싸 퀸", weight: 4, min: 4, why: "진 판의 BEST PLAYER", unit: "회", value: (s) => ((s.lostBests ?? 0) > 0 ? s.lostBests! : null) },
