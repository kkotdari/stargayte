...
const TITLES: Title[] = [
  ...
  { ...tactic("동맹을 지키는 포탑", ["ally-cannon"]), minPlaysShare: 0.09 },
  ...
  { ...tactic("집요한 일꾼 헌터", ["harass-workers"], 1), minPlaysShare: 0.09 },
  ...
  { ...tactic("목동저그", ["moka"]), minPlaysShare: 0.025, vsWins: true },
  ...
  { ...tactic("오버로드 사냥꾼", ["valk-hunt"]), race: undefined, minPlaysShare: 0.09 },
  ...
  { ...rare("다크스웜 살포반", ["swarm"]), minPlaysShare: 0.03, vsWins: true },
  ...
  { ...tactic("노엘을 외치는 여인", ["no-elim"]), minPlaysShare: 0.08 },
     ...
     말인데, 채우는 것이 "제일 먼저 죽었다"면 비워 두는 편이 낫다. */
  {
    ...
    value: (_s, of) => {
      /* 종족당 판수 문턱 12 → 18(지적: 팔색조 문턱이 낮다) — 종족 수는 셋이 끝이라 더 올릴
         ...
         한 시즌을 산 것이다. */
      const enough = Object.values(of.races ?? {}).filter((st) => (st?.plays ?? 0) >= 15);
