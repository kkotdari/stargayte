...
function signature(
  ...
  const says = s.plays >= MIN_PLAYS_RATE && s.winRate >= 60
    ? [
      ...
      "지는 법을 잊은 사람", "웃으면서 이기는 사람", "오늘도 한 판 챙김",
    ]
    : s.winRate <= 35
      ? [
        "다음 판을 노리는 자", "복수를 준비하는 자", "칼을 가는 중", "곧 터질 사람",
        "아직 각성 전", "다음 판이 진짜다", "포기를 모르는 사람", "리벤지 예약",
      ]
      : [
        "묵묵히 한 판 더", "꾸준함이 무기", "한 판 더의 사람", "성실한 출석부",
        "늘 자리를 지키는 사람", "오늘도 출근", "숨은 실력자", "조용한 잠재력",
        "다음 판의 주인공", "곧 칭호가 붙을 사람", "무던한 승부사", "그저 게임이 좋은 사람",
      ];
