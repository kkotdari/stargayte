...
const TEMPLATES: Record<string, Tpl> = {
  ...
  //
  // 수는 '경기 내내 뽑은 누계'다 — 죽은 것이 빠지지 않으므로 "N기를 모았다"처럼 한때
  // 동시에 그만큼 굴렸다는 말로 쓰면 안 된다(지적). 뽑은 수로만 말한다.
  bc: (c) => {
    const n = num(c.p.n, 3);
    return `${ga(c.who)} ${done(c, c.pick([
      `배틀크루저를 ${n}기까지 뽑음`,
      `끝내 배틀크루저까지 올림`,
      `배틀크루저를 ${n}기 뽑아 하늘로 밀고 나감`,
    ]), true)}`;
  },

  // 캐리어(요청) — 배틀크루저·목동과 같은 자리. 위 bc와 같은 이유로 '모았다'가 아니라
  // '뽑았다'로 말한다. 캐리어는 6서플이라 동시 보유 상한이 서른셋인데, 다른 유닛과 같이
  // 뽑는 이상 실제로 함께 떠 있던 수는 누계보다 한참 적다.
  carrier: (c) => {
    const n = num(c.p.n, 4);
    return `${ga(c.who)} ${done(c, c.pick([
      `캐리어를 ${n}기까지 뽑아 하늘을 노림`,
      `캐리어를 ${n}기 뽑아 올림`,
      `캐리어 ${n}기로 승부를 걺`,
      // 수가 적을 때만 '끝내 올렸다'로 말한다 — 서른 기 넘게 뽑은 경기에서 수를 빼면
      // 정작 그 경기의 그림이 사라진다.
      ...(n < 8 ? ["끝내 캐리어까지 올림"] : []),
    ]), true)}`;
  },
