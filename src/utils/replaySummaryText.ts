...
const TEMPLATES: Record<string, Tpl> = {
  ...
  // 성큰 러쉬 — 내 기지가 아닌 곳에 초반부터 성큰을 박는 올인(요청). 해처리는 펴지 않는다.
  "sunken-rush": (c) => {
    const at = targetPhrase(c);
    return `${ga(c.who)} ${at}${c.pick(
      c.won
        ? ["성큰 러쉬로 밀어붙임", "코앞에 성큰을 박아 숨통을 조임", "본진 밖에 성큰을 박고 조여 들어감"]
        : ["성큰 러쉬를 갔지만 막힘", "성큰 러쉬 갔다가 손해만 봄", "성큰 러쉬 야심차게 갔다가 망함"]
    )}`;
  },
  ...
  // 옆탱(요청) — 아군 기지에 팩토리를 올려 탱크로 그쪽을 받쳐주는 팀전 플레이.
  "side-tank": (c) => {
    const at = c.who2 ? `${c.who2}의 기지에 ` : "아군 기지에 ";
    return `${ga(c.who)} ${at}${c.pick(
      c.won
        ? ["팩토리를 올려 옆탱으로 받쳐줌", "옆탱을 박아 그쪽 라인을 잡아줌", "팩토리를 펴고 탱크로 문을 걸어줌"]
        : ["옆탱을 박았지만 같이 무너짐", "옆탱까지 갔지만 라인이 버티지 못함"]
    )}`;
  },
