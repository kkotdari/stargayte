...
const TEMPLATES: Record<string, Tpl> = {
  ...
  moka: act([
    "저글링·울트라에 다크스웜을 얹은 목동 저그로 감", "울트라까지 모아 목동 저그를 굴림",
  ...
  bionic: (c) =>
    `${ga(c.who)} ${c.pick(
      c.p.tank
        ? ["마린·메딕에 탱크까지 붙인 바이오닉으로 감", "탱크를 붙인 바이오닉을 굴림"]
        : ["마린·메딕 바이오닉으로 감", "마린·메딕을 모아 바이오닉을 굴림"]
    )}${tail(c)}`,
  mech: act([
    "탱크와 골리앗을 앞세운 메카닉으로 감", "메카닉으로 자리를 잡고 나감", "메카닉을 굴림",
  ]),
  valkyrie: act([
