...
export interface UnitEnt {
  ...
  /** 죽음의 갈래 — tag(태그가 바뀜) · atk(맞아 죽음) · morph(변태) · cxl(취소) ·
   *  cap(인구 상한이 물린 합성 개체: 애초에 없던 것이라 죽는 장면이 없다). */
  dk: "tag" | "atk" | "morph" | "cxl" | "cap" | "";
  /** 건물이면 1. */
  bld: 0 | 1;
...
export function buildUnitTracks(
     ...
     [태그 재사용] 같은 태그의 여러 생애 중 그 선택 시각에 살아 있던 것만 본다. */
  {
    ...
    for (const life of infantryGuess) {
      ...
      if (!kind9) continue;
      /* ★ 몫을 넘겨서는 안 준다(계측으로 뒤집은 자리) — 한때 ④처럼 넘겨서라도 줬는데,
         총량으로 재 보니 마린이 뽑음 229기에 개체 292기였다. 63기가 이 어림에서 왔다.
         그 어림에는 순환이 있다: 정체를 모르는 생애의 생산 건물을 '종족 기본 생산소
         (배럭·게이트)'로 놓고, 다시 "배럭에서 나왔으니 기본 보병"이라고 이름을 준다.
         모르는 것에서 모르는 것을 낳는 셈이다.
         "원장이 덜 세는 것 아닌가"도 재 봤다 — 헛침(IneffKind=1)으로 버린 Train 155건을
         살려 보니 원장 1367 → 1522인데 결합률 70.7% → 63.5%, 합성 364 → 515으로
         나빠졌다. 그 커맨드들은 진짜로 유닛이 안 됐다. 원장은 덜 세고 있지 않다.
         그래서 몫 안에서만 준다. 대가는 무명률이고(90300 1.0 → 1.8%), 얻는 것은
         지어낸 이름이 없다는 것이다. */
      if ((quota.get(life.owner)?.get(kind9) ?? 0) <= 0) { infOver += 1; continue; }
      bump(life.owner, kind9, -1);
    ...
    for (const [, mine] of byOwner) {
      ...
      for (let t = 60; t <= endSec; t += STEP) {
        ...
        for (const u of syn) {
          ...
          u.d = Math.round(t);
          /* 갈래는 'atk'가 아니라 'cap'이다(지적: "복제품들이 땅에 나타나서는 곧 혼자
             죽음") — 이건 맞아 죽은 것이 아니라 **원장이 제 과잉 계상을 무르는 것**이다.
             합성 개체는 '뽑힌 기록은 있는데 관측을 못 한 유닛'을 메우려고 지어 낸 것인데,
             인구가 "이만큼은 살아 있을 수 없다"고 말하면 그중 몇은 애초에 없던 것이다.
             없던 것이 죽는 장면은 없어야 한다 — 재생기가 이 갈래에는 사망 효과를 안
             그린다(실측 scripts/drop-check.mjs: 죽은 합성 114기 중 94기가 곁에 적
             공격이 하나도 없는 죽음이었다). */
          u.dk = "cap";
          used -= SUPPLY_COST[u.k] ?? 2;
          retired += 1;
