...
export interface ReplaySummaryData {
  ...
  /* (삭제·요청: v1 이전 스토리 요약 데이터 정리) — 옛 키 mvp(BEST의 옛 이름)와
     spots(시작 지점 시계 호칭)는 쓰기도 읽기도 없다. 이미 저장된 옛 행의 값은 재분석이
     summary_data를 다시 쓰면서 사라진다. mvp만 bestRawOf가 폴백으로 읽는다. */
...
export function bestRawOf(s: ReplaySummaryData | null | undefined): string | undefined {
  // 옛 행의 mvp 키는 재분석 전까지만 남는다 — 타입에서 걷어낸 뒤라 폴백으로만 읽는다.
  return s?.best ?? (s as { mvp?: string } | null | undefined)?.mvp;
}
