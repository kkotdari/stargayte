...
export function scenePace(sec: number): number {
  /* 후반 상한을 1.5 → 1.25로 낮추고 오르는 기울기도 눕혔다(지적: 이제는 후반 전투가 너무
     묶인다. 살짝 완화 필요) — 실측 비율(20분 이후 1.51)을 그대로 따라갔더니, 창이 2분을
     넘겨 실제로는 따로 벌어진 후반 교전들이 한 문장으로 붙었다. 창을 넓히는 쪽은 붙일지
     말지를 다른 조건들(같은 사람·같은 전황)이 함께 보므로 조금 짧게 잡는 편이 안전하다.
     초반을 좁히는 쪽(0.6)은 그대로 둔다 — 그쪽은 실측대로 서로 다른 장면이 붙는 문제였다. */
  return Math.min(1.25, Math.max(0.6, 0.55 + sec / (20 * 60)));
}
