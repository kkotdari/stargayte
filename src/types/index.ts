...
// 4개 상태만 있다 — 응답대기(pending)/성사(confirmed, 결투 대기)/완료(done)/폐기(discarded,
...
// 도전자 쪽/지목된 쪽 — 재결투 신청 자격 판정(패배한 쪽) 등에 쓰인다.
export type ChallengeSide = "creator" | "target";
// 확정 결투의 결과 — 이긴 쪽(creator/target) 외에 무승부(draw)/미실시(not_held)도 있다.
...
// 재결투 체인에서 이 도전장보다 앞선(더 예전) 기록 한 건.
export interface ChallengeHistoryEntry {
  ...
  createdAt: string;
  // 확정 결투의 결과 — 아직 아무도 입력하지 않았으면 null.
...
export interface Challenge {
  ...
  // 재결투(설욕전)로 만들어졌으면 원래 도전장의 id, 아니면 null. 값이 있으면 곧 재결투다.
  reappliedFromId: number | null;
  // 확정 결투의 결과 — 아직 아무도 입력하지 않았으면 null.
  resultWinnerSide: ChallengeResult | null;
  // 이 도전장보다 앞선 체인 기록(오래된 순) — 목록 화면 카드에서 좌우로 슬라이드해
  // 보여준다. 재결투 이력이 없으면 빈 배열.
  history: ChallengeHistoryEntry[];
  // "결투 신청 들어주기"로 만들어졌으면 true — 카드에 "요청결투" 배지를 붙인다.
...
export interface ChallengeCreatePayload {
  ...
  // "결투 신청 들어주기"로 여는 도전장이면 true.
  fromMatchRequest?: boolean;
}

// 재결투(설욕전)을 신청할 때 — 시간은 생략할 수 있다.
