...
export default function CreateMemberModal({ onClose }: CreateMemberModalProps) {
  ...
  const submit = async () => {
    if (!id || !pw || !tag) { setErr("필수 항목(*)을 모두 입력해 주세요."); return; }
    setErr("");
    setBusy(true);
    try {
      await createMemberByAdmin({
        id, password: pw, battletag: tag,
        ...
        <div className="scr-modal-body">
          ...
          <div className="scr-avatar-pick">
            <div className="scr-avatar-pick-media">
              <Avatar member={{ id: id || "new", nickname: tag || "?", avatar }} size={56} />
            </div>
          ...
          <label className="scr-field">
            ...
            <input className="scr-input" value={id} onChange={(e) => setId(e.target.value)} placeholder="사용할 아이디" />
          </label>
          <label className="scr-field">
            <span className="scr-label">비밀번호 *</span>
            <input className="scr-input" type="password" value={pw} onChange={(e) => setPw(e.target.value)} />
          </label>
          <label className="scr-field">
