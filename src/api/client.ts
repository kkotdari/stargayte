...
export const api = {
  ...
  // 위의 "결과 입력" 버전 — 내가 참가한 확정 챌린지 중 예정 일시가 지났는데 아직 결과가
  ...
  // 확정된 챌린지의 결과(이긴 쪽)를 입력 — 참가자 누구든 먼저 입력하는 쪽이 인정되고,
  // 예정 일시가 지난 뒤에만 가능하다. 이미 결과가 입력된 챌린지에는 다시 입력할 수 없다.
  ...
  // 챌린지가 성사되면 작성자 본인/운영자가 "성사됨"으로 완료 처리한다(목록에서 사라짐).
