...
import { bodyFace, sideFace, topFace, type ShapeFace } from "../../utils/shapeOblique";
...
const ZERG_MOUND =
  ...
  /* 반원통 다리(지적: 아직도 바닥이 둥글어 보임) — 바닥 변은 완전한 수평 직선(L)이고
     ...
     갈퀴는 좁고 가파른 마루로만 솟는다. */
  /* 앞줄 세 다리(지적: 여섯 다리면 앞 셋·뒤 둘만 보여야 한다) — 바닥 수평 직선 + 작은
     모서리, 사이 갈퀴는 좁은 마루. */
  + " M11.5 10"
  + " Q13.2 10.6 13.4 12.2 Q13.4 13.9 12.6 13.9 L10.9 13.9 Q10.4 13.9 10.3 13.2"
  + " Q10.1 12.8 9.8 13.1"
  + " Q9.7 14.3 9 14.3 L7.6 14.3 Q7 14.3 6.9 13.3"
  + " Q6.7 12.9 6.4 13.2"
  + " Q6.3 13.9 5.6 13.9 L3.9 13.9 Q3.1 13.9 3.1 12.4"
  + " Q3.3 10.7 4.5 10"
  + " Q8 11.4 11.5 10 Z"
  /* 뒷줄 두 다리 — 몸통 옆으로 빼꼼 보이는 토막(위 둥근 등·평평한 바닥, 앞줄보다 높은
     바닥선이라 뒤로 물러나 보인다). */
  + " M13.6 10.2 Q15.4 10.4 15.7 11.7 Q15.8 12.5 15.1 12.5 L13.9 12.5 Q13.3 12.5 13.2 11.4 Q13.2 10.4 13.6 10.2 Z"
  + " M2.4 10.2 Q0.6 10.4 0.3 11.7 Q0.2 12.5 0.9 12.5 L2.1 12.5 Q2.7 12.5 2.8 11.4 Q2.8 10.4 2.4 10.2 Z";
/** 저그 본진 머리의 평평한 윗면(요청) — 둥근 머리 위 밝은 타원. 더 얇게(지적: 뚜껑). */
// 윗면은 잘린 윗둥 너비만큼 넓게(요청: 높이는 줄이고 윗면은 넓게).
const ZERG_TOP = "M6 5.65a2 0.5 0 1 0 4 0a2 0.5 0 1 0-4 0Z";
...
const ZERG_CAP_RF = "M10.95 13.95 L12.55 13.65 Q12.5 12.75 11.65 12.8 Q10.85 12.95 10.95 13.95 Z";
const ZERG_CAP_CF = "M7.2 14.25 L8.9 14.25 Q9 13.2 8.05 13.2 Q7.1 13.2 7.2 14.25 Z";
const ZERG_CAP_LF = "M3.95 13.65 L5.55 13.95 Q5.65 12.95 4.85 12.8 Q4 12.75 3.95 13.65 Z";
const ZERG_CAP_RB = "M14 12.5 L15.1 12.2 Q15 11.5 14.4 11.55 Q13.9 11.7 14 12.5 Z";
const ZERG_CAP_LB = "M0.9 12.2 L2 12.5 Q2.1 11.7 1.6 11.55 Q1 11.5 0.9 12.2 Z";
...
const SHAPE_FACES: Record<string, ShapeFace[]> = {
  ...
  lair: [
    [`${ZERG_MOUND}`
      + " M0.5 11.9 Q0.1 8 1.2 5.2 Q2 8.4 2.4 12.1 Z"
  ...
  hive: [
    [`${ZERG_MOUND}`
      + " M0.4 11.9 Q-0.1 7 1.1 2.8 Q2.1 7.6 2.5 12.1 Z"
      + " M7 14 Q6.6 8.6 7.8 3.6 Q8.9 9 8.8 14.2 Z"
