...
export const SHAPE_BUILDERS: Record<string, () => ShapeFace[]> = {
  ...
  dship: () => [
    // 꼬리날개 — 맨 뒤라 먼저.
    ...hornFaces(0, -3.2, 3.6, 0, -4.3, 7.4, 1.1),
    // 엔진 포드 — 뒤 양옆의 굵은 통(앞단면 둥글게).
    ...tubeFaces(-2.7, -3.8, -2.7, -0.4, 1.25, 3.4, true),
    ...tubeFaces(2.7, -3.8, 2.7, -0.4, 1.25, 3.4, true),
    // 짧은 날개 — 포드 앞에서 밖으로 젖혀진 판.
    bodyFace(polyPath3([[-1.8, 0.6, 3.4], [-4.4, -0.6, 3.3], [-4.2, -1.6, 3.3], [-1.8, -0.8, 3.4]])),
    bodyFace(polyPath3([[1.8, 0.6, 3.4], [4.4, -0.6, 3.3], [4.2, -1.6, 3.3], [1.8, -0.8, 3.4]])),
    sideFace(polyPath3([[1.8, 0.6, 3.4], [4.4, -0.6, 3.3], [4.2, -1.6, 3.3], [1.8, -0.8, 3.4]]), 0.2),
    // 몸통 — 각진 상자.
    ...boxFaces3(0, -0.6, 3.4, 5, 2.1, 2.6),
    // 앞코 — 정면(+y)으로 좁아지며 내려가는 쐐기.
    bodyFace(polyPath3([[-1.7, 1.9, 4.7], [1.7, 1.9, 4.7], [0.9, 4.6, 3], [-0.9, 4.6, 3]])),
    topFace(polyPath3([[-1.7, 1.9, 4.7], [1.7, 1.9, 4.7], [0.9, 4.6, 3], [-0.9, 4.6, 3]]), 0.18),
    // 조종석 창 — 코 위 어두운 띠.
    capFace(polyPath3([[-0.9, 2.6, 4.35], [0.9, 2.6, 4.35], [0.6, 3.6, 3.7], [-0.6, 3.6, 3.7]]), 0.45),
  ],
  /* 셔틀(다시, 실물 참고) — 집게발쪽이 정면(지적): 넓적한 방패꼴 몸통의 앞(+y)으로
     집게 두 갈래가 뻗고 사이가 파여 있다. 가운데 앞쪽 콕핏 혹, 뒤 엔진 줄. */
  shuttle: () => {
    const [cx, cy] = project(0, -0.6, 3.8);
    const claw = (side: 1 | -1): string => polyPath3([
      [side * 2.5, 1.2, 3.9], [side * 1.9, 4.4, 3.9], [side * 0.9, 5, 3.9],
      [side * 0.7, 3.2, 3.9], [side * 1, 1.6, 3.9],
    ]);
    const [ex, ey] = project(0, -3.3, 3.9);
    return [
      // 뒤 엔진 줄 — 몸통이 뿌리를 덮는다.
      topFace(groundEllipse(ex, ey, 1.7, 0.5), 0.3),
      // 몸통 — 넓적한 방패꼴.
      bodyFace(`M${cx - 3.8} ${cy} Q${cx - 3.4} ${cy - 2.4} ${cx} ${cy - 2.7} Q${cx + 3.4} ${cy - 2.4} ${cx + 3.8} ${cy} Q${cx + 3.1} ${cy + 2.5} ${cx} ${cy + 2.8} Q${cx - 3.1} ${cy + 2.5} ${cx - 3.8} ${cy} Z`),
      sideFace(`M${cx + 1.2} ${cy - 2.5} Q${cx + 3.6} ${cy - 1.6} ${cx + 3.4} ${cy + 1.4} Q${cx + 3.4} ${cy - 1} ${cx + 1.2} ${cy - 2.5} Z`, 0.2),
      topFace(groundEllipse(cx - 1.2, cy - 1.2, 1.6, 1), 0.25),
      // 집게 — 정면(+y) 두 갈래와 사이의 어두운 홈.
      bodyFace(`${claw(1)} ${claw(-1)}`),
      topFace(claw(-1), 0.16),
      sideFace(claw(1), 0.16),
      capFace(polyPath3([[0.8, 1.6, 3.92], [0, 4.2, 3.92], [-0.8, 1.6, 3.92]]), 0.4),
      // 콕핏 혹 — 가운데 앞쪽.
      ...domeFaces3(0, 0.6, 1.05, 0.8, 3.9),
    ];
  },
};
