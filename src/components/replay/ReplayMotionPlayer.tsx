...
export const SHAPE_BUILDERS: Record<string, () => ShapeFace[]> = {
  ...
  /* 히드라리스크(실물 참고) — 똬리 감은 마디 꼬리 위에 세운 몸, 갈빗대 배, 낫팔 한 쌍,
     작은 머리 뒤로 크게 흐르는 두건 볏. */
  hydra: () => {
    const hood = polyPath3([[0.5, 0.4, 7.2], [1, -1.6, 8.7], [0, -2.9, 9], [-1, -1.6, 8.7], [-0.5, 0.4, 7.2]]);
    return [
      // 똬리 꼬리 — 겹친 마디 캡슐.
      ...tubeFaces(-1.4, -1.9, 1.2, -2.3, 0.75, 3.3),
      ...tubeFaces(-1.3, -1, 1.5, -1.2, 0.8, 3.4),
      // 세운 몸통 + 갈빗대 배.
      ...cylinderFaces3(0, -0.2, 1.05, 3, 3.8),
      capFace(polyPath3([[-0.6, 0.83, 4.6], [0.6, 0.83, 4.6], [0.55, 0.83, 4.85], [-0.55, 0.83, 4.85]]), 0.2),
      capFace(polyPath3([[-0.55, 0.83, 5.3], [0.55, 0.83, 5.3], [0.5, 0.83, 5.55], [-0.5, 0.83, 5.55]]), 0.2),
      capFace(polyPath3([[-0.5, 0.83, 6], [0.5, 0.83, 6], [0.45, 0.83, 6.25], [-0.45, 0.83, 6.25]]), 0.2),
      // 낫팔 한 쌍.
      ...claw3(1, 0.9, 5),
      ...claw3(-1, 0.9, 5),
      // 작은 머리 + 두건 볏.
      ...domeFaces3(0, 0.2, 0.75, 0.6, 6.9),
      bodyFace(hood),
      topFace(hood, 0.14),
    ];
  },
  ...
  /* 디파일러(실물 참고) — 아치로 솟은 마디 등(가로 띠), 앞 낮은 곤충 머리와 긴 더듬이
     한 쌍, 잔다리들, 짧은 꼬리. */
  defiler: () => [
    // 잔다리.
    ...hornFaces(-1.1, -0.6, 3.4, -2, -0.3, 1.9, 0.42),
    ...hornFaces(1.1, -0.6, 3.4, 2, -0.3, 1.9, 0.42),
    ...hornFaces(-1, 0.6, 3.2, -1.8, 1, 1.8, 0.4),
    ...hornFaces(1, 0.6, 3.2, 1.8, 1, 1.8, 0.4),
    // 아치 마디 등 — 가운데가 높은 가로 띠들.
    ...tubeFaces(-1.2, -2.4, 1.2, -2.4, 0.6, 2.9),
    ...tubeFaces(-1.4, -1.4, 1.4, -1.4, 0.7, 3.8),
    ...tubeFaces(-1.5, -0.3, 1.5, -0.3, 0.75, 4.3),
    ...tubeFaces(-1.4, 0.8, 1.4, 0.8, 0.7, 3.8),
    // 앞 낮은 머리 + 긴 더듬이 한 쌍.
    ...domeFaces3(0, 1.9, 0.9, 0.7, 3),
    ...hornFaces(0.3, 2.5, 3.6, 1.9, 4.4, 5.2, 0.22),
    ...hornFaces(-0.3, 2.5, 3.6, -1.9, 4.4, 5.2, 0.22),
    // 짧은 꼬리.
    ...hornFaces(0, -2.8, 3, 0.3, -3.8, 3.5, 0.45),
  ],
