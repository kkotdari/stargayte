...
export const SHAPE_BUILDERS: Record<string, () => ShapeFace[]> = {
  ...
  citadel: () => {
    /* 시타델 오브 아둔(실물 참고) — 둥근 드럼 몸통에 각진 볏 머리, 양옆으로 뻗은 팔
       끝에 세로 패들 지느러미가 달린 특징적인 꼴. 앞면엔 렌즈 점들. */
    ...
    return [
      // 양옆 팔 + 패들 — 왼쪽 먼저(뒤로 살짝 젖힘).
      ...tubeFaces(-2.2, -0.2, -4.4, -0.2, 0.45, 3.4),
      ...boxFaces3(-4.8, -0.2, 0.6, 2.5, 2.9, 2),
      ...tubeFaces(2.2, -0.2, 4.4, -0.2, 0.45, 3.4),
      ...boxFaces3(4.8, -0.2, 0.6, 2.5, 2.9, 2),
      // 몸통 — 낮은 받침 위 둥근 드럼.
      ...cylinderFaces3(0, 0, 2.9, 1.2),
      ...domeFaces3(0, 0, 2.75, 2.5, 1.2),
      lens(-1.1, 2.3, 2.2), lens(0.2, 2.5, 2), lens(1.4, 2.2, 2.3),
      // 각진 볏 머리 — 위에 얹혀 뒤로 솟는다.
      ...boxFaces3(0, -0.3, 1.9, 1.7, 1.7, 3.6),
      ...hornFaces(0, -0.9, 5.2, 0, -1.8, 6.8, 0.9),
      lens(0, 0.7, 5.4),
    ];
  },
  ...
  hydraden: () => {
    ...
    return [
      ...
      /* 갈퀴막은 네 폭(지적) — 기둥 셋이 펼쳐지니 사이 두 폭에 바깥쪽 두 폭까지,
         모두 바닥에 닿는다. */
      bodyFace(polyPath3([[-4.2, -2.5, 7], [-5.5, -1.6, 0.3], [-3.2, -0.9, 0.3]])),
      sideFace(polyPath3([[-4.2, -2.5, 7], [-5.5, -1.6, 0.3], [-3.2, -0.9, 0.3]]), 0.14),
      ...
      bodyFace(polyPath3([[4, -2.9, 6.5], [5.3, -1.8, 0.3], [3, -1.1, 0.3]])),
      sideFace(polyPath3([[4, -2.9, 6.5], [5.3, -1.8, 0.3], [3, -1.1, 0.3]]), 0.22),
      // 앞 똬리 꼬리 — 마디 돔 줄 + 끝 입.
  ...
  goliath: () => [
    // 기계식 꺾인 다리(지적) — 엉덩이→무릎(뒤로), 무릎→발목(앞으로) 두 마디 + 발판.
    ...hornFaces(-1.2, -0.2, 5.6, -1.7, -1.3, 3.4, 0.8),
    ...hornFaces(-1.7, -1.3, 3.4, -1.9, 0.7, 1, 0.65),
