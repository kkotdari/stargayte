...
// (이동·재지적) 앞 톱니 원판 — 왼쪽 반원판의 이빨로 옮겼다.
  ...
  evo: () => {
    ...
    return [
      /* 사각기둥 제거(재지적) — 그 자리(0.7,-1.9)에 혈관 파이프들이 수직으로 선다:
         높이·기울기 다른 캡슐 네 가닥. */
      ...rodFaces(0.4, -1.7, 0.2, 0.3, -1.85, 4.3, 0.6),
      ...rodFaces(1.15, -2.1, 0.2, 1.35, -2.25, 3.4, 0.5),
      ...rodFaces(0.75, -1.45, 0.2, 0.95, -1.25, 2.8, 0.45),
      ...rodFaces(0.15, -2.35, 0.2, -0.05, -2.55, 2.3, 0.4),
  ...
  scout: () => {
    ...
    return [
      // 날개만 빼고 은색(요청).
      ...paintBase(tubeFaces(-0.55, -3.1, -0.55, -2, 0.3, 5.9), "#c9ced6"),
      ...paintBase(tubeFaces(0.55, -3.1, 0.55, -2, 0.3, 5.9), "#c9ced6"),
      bodyFace(wingL), topFace(wingL, 0.16),
      bodyFace(wingR), sideFace(wingR, 0.18),
      [hull, 1, "#c9ced6"] as ShapeFace, topFace(hull, 0.16),
      ...paintBase(domeFaces3(0, 0.9, 0.6, 0.5, 6.3), "#c9ced6"),
    ];
  },
  ...
  zealot: () => [
    ...
    ...((): ShapeFace[] => {
      ...
      return [
        bodyFace(`M${hx - 0.58} ${hy} A0.58 0.45 0 0 1 ${hx + 0.58} ${hy}`
          + ` Q${hx + 0.5} ${hy + 0.5} ${cxx} ${cyy} Q${hx - 0.5} ${hy + 0.5} ${hx - 0.58} ${hy} Z`),
  ...
  zling: () => [
    // 꼬리·다리 검회색, 발톱 상아색 유지(요청).
    ...paintBase(hornFaces(0, -1.6, 3.6, 0, -3.2, 3.3, 0.5), "#3a3f46"),
    ...paintBase(hornFaces(-0.9, -0.8, 3.6, -1.6, -1.4, 2.1, 0.45), "#3a3f46"),
    ...
    ...paintBase(hornFaces(-0.8, 0.6, 3.4, -1.3, 1.4, 2, 0.4), "#3a3f46"),
