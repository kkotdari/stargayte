...
// (이동·재지적) 앞 톱니 원판 — 왼쪽 반원판의 이빨로 옮겼다.
  ...
  muta: () => {
    ...
    const wing = (m2: 1 | -1): ShapeFace[] => {
      // 위아래 뒤집기(재지적) — 뿌리에서 내려갔다 오르고 끝이 처지는 지그재그.
  ...
  carrier: () => {
    ...
    return [
      ...
      /* 옆 두 잎의 개인색 장식은 아랫면 바깥 절반에만(재재지적: 안쪽으로 삐져나옴) —
         중심을 바깥(±1.75)으로 옮기고 폭을 줄여 잎 안변을 안 넘는다. */
      ...paintBase(lens(-1.3, -1, 5.1, 1.05, 3.9, "L"), "#d4af37"),
      [petal(-1.75, -1, 4.62, 0.38, 1.5), 0.4] as ShapeFace,
  ...
  zealot: () => [
    ...
    // 어깨 뽕 한 쌍 — 몸과 같이 금색(재재지적).
    ...paintBase(domeFaces3(-1.35, -0.25, 0.62, 0.5, 6), "#d4af37"),
    ...paintBase(domeFaces3(1.35, -0.25, 0.62, 0.5, 6), "#d4af37"),
    // 상완은 고유색(재재지적: 상완·대퇴만 고유색).
  ...
  dtemp: () => [
    ...
    ...rodFaces(1.05, -0.15, 5.7, 1.7, 0.3, 4.7, 0.5),
  ...
  htemp: () => {
    ...
    return [
      ...
      ...paintBase(hornFaces(0.8, -1, 7, 1.4, -1.2, 9.4, 0.4), "#d4af37"),
      ...paintBase(hornFaces(1.4, -1.2, 9.3, 0.35, -1.35, 10.2, 0.3), "#d4af37"),
      // 두 손을 들고 다닌다(요청) — 팔(상완)은 고유색(재재지적).
