...
// (이동·재지적) 앞 톱니 원판 — 왼쪽 반원판의 이빨로 옮겼다.
  ...
  devourer: () => [
    ...
    ...((): ShapeFace[] => {
      ...
      const segs9: [number, number, number, number][] = [ // [y, 반지름, 높이, 바닥 z]
        /* 세 마디 모두 앞으로(재지적: 뒤로 너무 튀어나옴) — 간격은 서로 겹치도록
           좁게 유지해 마디가 끊기지 않는다. */
        [1.55, 0.85, 0.85, 1.35], [0.35, 1.15, 1.1, 2.1], [-0.75, 1.5, 1.35, 2.95],
      ];
    ...
    // 등판 이음선.
    sideFace(polyPath3([[0, 1.55, 2.1], [0, 0.2, 3.3], [0, -0.4, 6.3], [0.16, -0.4, 6.3], [0.16, 0.2, 3.3], [0.16, 1.55, 2.1]]), 0.16),
