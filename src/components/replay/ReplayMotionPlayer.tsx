...
import {
  ...
  boxFaces3, cylinderFaces3, discPath3, polyPath3, prismXFaces, pyramidFaces3, project,
...
const ZERG_MOUND =
  /* 3D 재작도(요청: 모형을 세워 놓고 요잉 −, 피칭 +) — 여섯 다리(60° 간격)를 요잉 −20°,
     피칭 +(납작비 0.45)로 투영한 결과다:
       앞-왼(−20°) 짧고 단면 최대 · 앞-오른(+40°) 대각 중간 · 왼쪽(−80°) 가장 긴 옆모습
       + 단면 조각 · 오른쪽(+100°) 길지만 살짝 뒤라 단면 없음(끝이 위로) · 뒤-왼(−140°)
       어깨 위로 크게 · 뒤-오른(160°) 몸통 뒤라 작게. 전부 시계 감김(구멍 방지). */
  "M5.4 5.7 Q8 5.3 10.6 5.7 Q10.9 10.4 14.2 12.6 Q8 13.8 1.8 12.6 Q5.1 10.4 5.4 5.7 Z"
  + " M11.2 10.4"
  + " Q12.4 11.2 13.3 12.9 Q13.7 13.7 13 13.9 L11.9 14.2 Q11.2 14.3 11 13.6"
  + " Q10.6 12.5 10 12.1"
  + " Q9.3 11.8 8.9 12.2"
  + " Q8.2 13.2 7.6 14.2 Q7.3 14.9 6.6 14.9 L5.6 14.7 Q5 14.5 5.2 13.8"
  + " Q5.5 12.8 5.8 12.1"
  + " Q5.4 11.7 5 12"
  + " Q4.3 12.3 2.6 12.4 Q1.5 12.4 1.2 12 L1 11.5 Q0.9 11 1.6 10.9"
  + " Q3 10.7 4.4 10.4"
  + " Q8 11.4 11.2 10.4 Z"
  /* 뒤 세 다리의 재투영(지적) — 뒤-왼은 몸통에 가려 안 그리고, 뒤-오른(+100°)은 오른쪽
     옆으로 온전히 보이는 긴 다리(살짝 뒤라 단면 없음·끝이 위로), 뒤-가운데(160°)는 오른
     어깨 너머 작은 등만. */
  /* 뿌리를 몸통 옆구리 안쪽까지 밀어 넣는다(지적: 뒤 다리가 떠 있는 혹처럼 보임).
     뒤-가운데 다리는 이 시점에선 거의 안 보인다(지적) — 아예 걷었다. 남는 건 뒤-오른
     (+100°) 긴 다리 하나다. */
  /* 반원통답게(지적) — 곧은 등마루(위)와 곧은 바닥선(아래)이 나란히 가고 끝만 둥글게
     말린 관 모양. 뒤로 갈수록 살짝 올라가며(피칭) 가늘어진다. */
  + " M11.6 10.3 L15.2 9.8 Q16.1 9.9 16 10.5 Q15.9 10.9 15.6 11.1 L11.8 11.9 Z";
...
const ZERG_HORN_SHADE_HIVE =
  "M5.6 2.6 Q4.9 6.6 5.8 10.4 L4.8 10.2 Q4.2 6.2 5.6 2.6 Z"
  + " M9.9 0.2 Q11.7 3.6 10.9 10.2 L10 10.2 Q10.7 4 9.9 0.2 Z"
  + " M14.7 0.9 Q16.2 5.6 14.2 11 L13.6 10.8 Q15.1 5.8 14.7 0.9 Z";
...
/* 입체(지적: 다리마다 보이는 방향이 다르다) — 단면의 바닥선을 그 다리가 뻗는 방향과
   직각으로 기울인다: 정면 다리는 반듯하게, 좌·우 앞다리는 안쪽이 낮게 기울고, 뒷다리는
   거의 옆모습이라 가늘고 비스듬한 조각만 보인다. */
const ZERG_CAP_RF = "M11 13.6 A0.95 0.9 0 0 1 12.85 13.85 Z";
// 앞 다리는 요잉 −20°라 뿌리도 정중앙이 아니라 살짝 왼쪽으로 돌아간 곳이다(지적).
const ZERG_CAP_CF = "M5.45 14.6 A1.02 0.97 0 0 1 7.4 14.75 Z";
const ZERG_CAP_LF = "M1.05 11.35 A0.55 0.5 0 0 1 1.35 12.35 Z";
...
// 면 규칙 표준은 utils/shapeOblique.ts에 있다(요청: 매번 고치지 않게) — 새 도형은 그 헬퍼로.
const SHAPE_FACES: Record<string, ShapeFace[]> = {
  ...
  /* 레어(정정) — 바닥 기둥 끝에서 위로 솟는 가시. 쪽마다 두 기둥 중 하나에만 나고,
     높이는 본(중심) 기둥보다 높다. */
  /* 레어(실물 참고) — 뒤 양쪽 다리 뿌리에서 안쪽으로 휘어 오르는 큰 엄니 둘. 끝의 밝은
     날은 topFace 조각이 맡는다. 엄니가 뒤에서 나와 앞다리 입구(캡)는 다 보인다. */
  lair: [
    [`${ZERG_MOUND}`
  ...
  /* 하이브 — 본 건물보다 훨씬 긴 뿔 셋이 위로 솟고, 가시는 그 뿔에서 본 건물 쪽(안쪽)
     으로 난다(지적). */
  /* 하이브(실물 참고) — 뱀목처럼 휘어 오르는 큰 뿔 셋(좌·뒤가운데·우), 레어보다 길고
     끝이 밝다. 앞바닥 양 모서리엔 숫양뿔 컬. */
  hive: [
    [`${ZERG_MOUND}`
      + " M3.4 10.8 Q1.7 4.6 4.3 0.9 Q5.7 0 7.4 0.7 Q6.2 1.3 5.6 3 Q4.9 6.6 5.8 10.4 Z"
      + " M2.7 7.7 L1.4 7 L2.9 6.2 Z M3 4.9 L1.9 4.1 L3.4 3.5 Z M3.9 2.5 L3.1 1.6 L4.5 1.4 Z"
      + " M8.5 10.4 Q8.5 3.4 9.9 0.2 Q11.7 3.6 10.9 10.2 Z"
      + " M8.6 7.4 L7.7 6.8 L8.7 6.2 Z M8.6 4.7 L7.8 4.1 L8.8 3.6 Z"
      + " M14.2 11 Q16.2 5.6 14.7 0.9 Q13 6 13.2 10.6 Z"
      + " M15.4 7.5 L16 6.7 L15.1 6.3 Z M15.2 4.6 L15.8 3.8 L14.9 3.5 Z"
    ...
    sideFace(ZERG_COLLAR, 0.35),
    topFace("M5.6 0.3 L7.4 0.7 L5.3 1.7 Z M9.3 0.9 L9.9 0.2 L10.6 1.1 L9.9 2 Z M14.2 1.6 L14.7 0.9 L15.4 1.9 L14.7 2.7 Z", 0.5),
    topFace(ZERG_TOP),
  ],
  /* 파일런 — 얇은 마름모 크리스탈의 허리를 둘러싼 납작한 고리(요청: "기둥을 둘러싼
     고리") — 토성 고리처럼 좌우로 삐져나온다. */
  /* 스포닝 풀(실물 참고 + 3D 규약) — 젤리는 봉긋한 방울이 아니라 바닥에 납작하게 깔린
     반투명 웅덩이(사선 납작비)다: 큰 것 하나(왼쪽)·작은 것 둘. 그 위를 가시 돋친 굵은
     힘줄 띠가 가로지르고 끝이 밖으로 삐져나온다. 힘줄 윗변 밝게·밑변 그늘(표준 음영). */
  pool: [
    sideFace("M1.4 12.4a6.6 1.5 0 1 0 13.2 0a6.6 1.5 0 1 0-13.2 0Z", 0.2),
    // 웅덩이 셋 — 반투명 젤리(뒤 지형이 비친다).
    ["M1 8.6a5.2 2.6 0 1 0 10.4 0a5.2 2.6 0 1 0-10.4 0Z", 0.55],
    ["M9.1 10.6a2.9 1.4 0 1 0 5.8 0a2.9 1.4 0 1 0-5.8 0Z", 0.55],
    ["M8.7 5.9a2.1 1 0 1 0 4.2 0a2.1 1 0 1 0-4.2 0Z", 0.55],
    // 물기 반짝임.
    ["M3.4 7.6a1.9 0.7 0 1 0 3.8 0a1.9 0.7 0 1 0-3.8 0Z", 0.3, "#fff"],
    ["M10.3 10.1a1.2 0.5 0 1 0 2.4 0a1.2 0.5 0 1 0-2.4 0Z", 0.3, "#fff"],
    // 힘줄 — 웅덩이를 가로지르는 굵은 띠들(불투명).
    ["M1.2 6.4 Q4.2 4.4 7.8 5 Q11.6 5.6 14.2 8.4 Q11.2 6.7 7.6 6.2 Q4 5.9 1.2 6.4 Z", 1],
    ["M1.6 10.9 Q5.4 12.9 9.2 12 Q12.4 11.2 14.4 11.8 Q12 12.9 9 13 Q5 13 1.6 10.9 Z", 1],
    ["M6.4 4.9 Q7.6 7.9 7 11.9 Q6.2 8.9 6.4 4.9 Z", 1],
    ["M13.6 8.2 Q14.6 9.6 14.2 11.6 Q13.6 10 13.6 8.2 Z", 1],
    // 가시 끝 — 밖으로 삐져나온 갈퀴.
    ["M1.5 6.2 L0.1 5.3 L1.8 7 Z", 1],
    ["M7.7 4.9 L8.4 3.3 L8.8 5.1 Z", 1],
    ["M14 8.3 L15.8 8.9 L14.2 9.4 Z", 1],
    ["M1.9 10.8 L0.4 11.9 L2.4 11.7 Z", 1],
    ["M13.9 11.9 L15.6 13 L13.5 12.6 Z", 1],
    // 힘줄 입체 — 윗변 밝게, 밑변 그늘.
    ["M1.6 6 Q4.4 4.5 7.8 5 Q6 4.9 3.6 5.4 Q2.2 5.7 1.6 6 Z", 0.3, "#fff"],
    sideFace("M7.6 6.2 Q11.2 6.7 14.2 8.4 Q11.6 7.6 8.4 7 Z", 0.3),
    sideFace("M1.6 10.9 Q5 13 9 13 Q5.6 13.4 3 12.3 Z", 0.3),
  ] as ShapeFace[],
  ...
  gate: (() => {
    ...
    return [
      ...
      // 꼭대기는 더 뭉뚝하게 + 높이 아주 살짝 낮게(요청 둘).
      ["M6.9 5.6 L9.1 5.6 L11 11.8 L5 11.8 Z", 1],
      ["M8 5.6 L9.1 5.6 L11 11.8 L8 11.8 Z", 0.22, "#000"],
      ["M6.9 5.6 L9.1 5.6 L9.3 6.2 L6.7 6.2 Z", 0.25, "#fff"],
  ...
  /* 드랍십 — 옆에서 본 뭉툭한 수송선(요청: 오버로드처럼 도형으로): 둥근 몸통, 오른쪽
     조종석 창, 왼쪽 위·아래 작은 핀. */
  /* 드랍십(요청) — 두 원통(엔진 포드)을 가운데 넓은 판형 통이 잇는 함선. 위에서 본
     모습: 위아래로 긴 원통 둘, 그 사이 넓적한 몸통 판. */
  dship: [
    bodyFace(
      "M3 3.4 L12 3.4 Q13.4 3.4 13.4 4.6 Q13.4 5.8 12 5.8 L3 5.8 Q1.6 5.8 1.6 4.6 Q1.6 3.4 3 3.4 Z"
      + " M3 10.2 L12 10.2 Q13.4 10.2 13.4 11.4 Q13.4 12.6 12 12.6 L3 12.6 Q1.6 12.6 1.6 11.4 Q1.6 10.2 3 10.2 Z"
      + " M4.2 5.4 L11.8 5.4 Q12.6 5.4 12.6 6.2 L12.6 9.8 Q12.6 10.6 11.8 10.6 L4.2 10.6 Q3.4 10.6 3.4 9.8 L3.4 6.2 Q3.4 5.4 4.2 5.4 Z",
    ),
    topFace("M3 3.8 L12 3.8 L12 4.3 L3 4.3 Z M3 10.6 L12 10.6 L12 11.1 L3 11.1 Z"),
    sideFace("M3.4 8.8 L12.6 8.8 L12.6 9.8 Q12.6 10.6 11.8 10.6 L4.2 10.6 Q3.4 10.6 3.4 9.8 Z", 0.25),
  ],
  /* 셔틀(요청) — 둥근 몸통에 앞부분(오른쪽)이 몽키스패너처럼 벌어진 두 집게. */
  shuttle: [
    bodyFace(
      "M1.8 8a4.3 3.5 0 1 0 8.6 0a4.3 3.5 0 1 0-8.6 0Z"
      + " M8.8 5.2 Q12.4 4.4 14.2 5.6 Q14.6 6.4 13.2 7 Q11.6 7.5 9.4 7.4 Z"
      + " M8.8 10.8 Q12.4 11.6 14.2 10.4 Q14.6 9.6 13.2 9 Q11.6 8.5 9.4 8.6 Z",
    ),
    sideFace("M2.2 8 Q2.6 10.8 5.6 11.3 Q3.8 9.8 3.8 8 Q3.8 6.2 5.6 4.7 Q2.6 5.2 2.2 8 Z", 0.25),
    topFace("M4.6 7a1.9 1 0 1 0 3.8 0a1.9 1 0 1 0-3.8 0Z"),
  ],
  ...
  // 공대공만 — 아래로 꺾인 V자 띠(요청: ^가 아니라 v 꼴 표식).
