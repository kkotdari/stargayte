...
// (이동·재지적) 앞 톱니 원판 — 왼쪽 반원판의 이빨로 옮겼다.
  ...
  muta: () => {
    ...
    return [
      /* 몸통을 스파이어 기둥으로(요청) — 겹비늘 마디 대신 한 몸으로 휘어 내려오는
         기둥. 끝은 뾰족하지 않게 뭉뚝히 남기고(tipW), 아래로 갈수록 앞으로 크게
         휜다. 기둥은 아래에서 위로 자라므로 '꼬리 끝(앞·아래)에서 머리(뒤·위)로'
         정의한다. */
      ...tagKey(spirePillar({
        x: 0, y: 1.5, z0: 3.5, h: 2.5, w: 0.34, tipW: 0.92,
        segs: 5, sides: 8, hold: 0,
        leanY: -1.1, curveY: -0.55,
        fill: "#6b4732",
      }), 12),
      // 머리 반구 — 기둥 위끝에 얹혀 앞을 본다.
      ...tagKey(paintBase(domeFaces3(0, -0.35, 1.05, 0.95, 5.9), "#6b4732"), 18),
  ...
  devourer: () => [
    ...
    ...((): ShapeFace[] => {
      /* 마디는 반구 바닥의 뒤쪽 가장자리에 붙어 시작한다(재지적) — 머리 반구 밑면
         (z 4.3)의 뒤끝(y -2.1)에서 첫 마디가 맞물리고, 뒤로 갈수록 아래로 내려간다. */
      /* 마디가 앞을 향해 강하게 휜다(요청) — 반구 뒤끝에서 아래로 내려가되, 아래로
         갈수록 y를 다시 앞(+)으로 크게 당겨 갈고리처럼 말린다. */
      /* 넷으로 늘려 휨을 완만하게(요청) — 마디마다의 꺾임이 작아져 곡선이 부드럽다.
         간격은 서로 겹치도록 좁게 유지해 마디가 끊기지 않는다. */
      /* 휨은 아래로 갈수록 심하게(재지적) — 위 두 마디는 거의 수직으로 내려가고,
         아래로 갈수록 y가 팍팍 앞으로 튄다(0.25 → 0.65 → 1.5 → 2.1). */
      const segs9: [number, number, number, number][] = [ // [y, 반지름, 높이, 바닥 z]
        /* 끝 마디가 떨어져 나오지 않게(지적) — 앞 마디와의 간격(y·z)이 각 마디
           반지름 합보다 작아야 겹친다. 휨은 아래로 갈수록 커지되 간격은 좁게. */
        [1.75, 0.72, 0.72, 1.2], [0.85, 0.98, 0.92, 1.75],
        [0.25, 1.25, 1.15, 2.4], [0, 1.5, 1.35, 3.05],
      ];
