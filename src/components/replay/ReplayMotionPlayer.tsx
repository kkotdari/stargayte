...
export const SHAPE_BUILDERS: Record<string, () => ShapeFace[]> = {
  ...
  /* 배틀크루저(합성 확정) — 처음의 장도리 몸(몸통+가로 머리+갈라진 두 발+함교)에,
     렌더판의 붐 날개(끝 거대 미사일)와 삼각형 추진기 셋만 합친 판. */
  bc: () => {
    const missile = (m2: 1 | -1): ShapeFace[] => [
      ...tubeFaces(m2 * 4.8, 0.4, m2 * 4.8, 3, 0.55, 5.3),
      ...hornFaces(m2 * 4.8, 3, 5.55, m2 * 4.8, 4.4, 5.5, 0.52),
      ...hornFaces(m2 * 5.15, 0.6, 5.6, m2 * 5.6, 0, 5.95, 0.3),
      ...hornFaces(m2 * 4.45, 0.6, 5.6, m2 * 4, 0, 5.95, 0.3),
    ];
    return [
      // 삼각 추진기 셋(아래 둘·위 가운데).
      ...tubeFaces(0, -3.7, 0, -2, 0.52, 6.7),
      ...tubeFaces(-0.9, -3.6, -0.9, -1.8, 0.6, 5.7),
      ...tubeFaces(0.9, -3.6, 0.9, -1.8, 0.6, 5.7),
      // 붐 날개 + 거대 미사일 한 발씩.
      ...hornFaces(-1.4, 0.2, 6, -4.5, 1.4, 5.7, 0.4),
      ...missile(-1),
      ...hornFaces(1.4, 0.2, 6, 4.5, 1.4, 5.7, 0.4),
      ...missile(1),
