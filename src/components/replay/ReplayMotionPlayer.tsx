...
export const SHAPE_BUILDERS: Record<string, () => ShapeFace[]> = {
  ...
  bc: () => [
    ...
    ...paintBase(prismYFaces(OCT_XZ(0, 6.02, 1.05, 0.9), 0.9, 2.8), "#c9ced6"),
    /* 띠 둘 — 관을 두르는 고리라 앞뒤 밑면을 둘 다 끈다. 목보다 0.15·0.13만 굵어 살짝
       ...
       그 둘을 합친 것보다 커야 한다. */
    ...tagKey(prismYFaces(OCT_XZ(0, 6.02, 1.2, 1.03), 1.35, 0.26, false, false),
      depthNow(0, 1.48) + 2.4),
    ...tagKey(prismYFaces(OCT_XZ(0, 6.02, 1.2, 1.03), 2.5, 0.26, false, false),
      depthNow(0, 2.63) + 2.4),
  ...
  fbat: () => {
    ...
    return [
      ...
      /* 몸은 붉다(요청: "파이어뱃 몸통 전체 빨간색") — 다리·몸통을 붉은 장갑색으로
         못 박는다. **어깨보호구만 빠진다**(요청: "파뱃 어깨보호구 개인색 적용") —
         칠하지 않은 면에 임자 색이 칠해지므로, 여기서 빼는 것이 곧 개인색 지정이다.
         이제 임자 색은 어깨보호구 한 쌍과 허리띠가 맡는다. */
      ...paintBase([
        // 다리를 또렷하게(지적: 다리가 없어 헷갈림) — 벌린 두 기둥 + 둥근 발.
        ...cylinderFaces3(-0.64, 0, 0.52, 2.3, 0.1),
        ...cylinderFaces3(0.64, 0, 0.52, 2.3, 0.1),
