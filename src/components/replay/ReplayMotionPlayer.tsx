  ...
  inf: () => {
    ...
    return [
      ...
      /* 헬멧은 **겉껍데기가 몸이고 얼굴가리개가 그 앞에 박힌다**(지적: "테란 보병 헬멧
         ...
         이제 겉껍데기가 머리를 감싸는 반구(제 색)이고, 얼굴가리개는 그 앞에 물려 박은
         반구다. 깊이 키도 **저마다 제 자리**에서 뽑아, 요잉이 돌면 앞뒤가 저절로 뒤집힌다.
         ★ 두 반구는 거의 같은 공이다(요청: "헬멧의 뒷부분 반구와 앞부분 고글 반구는
         반지름이 거의 똑같아 — 유리고글이 아주 살짝 작고, 고글의 뒷 반쪽을 헬멧이 딱 맞게
         덮고 있는 모양새"). 여태 가리개가 껍데기의 0.63배(0.62 대 0.98)뿐이라 큰 공에 작은
         공이 얹힌 눈사람으로 읽혔다.
         ★ 가리개는 뒤로 물리고 반투명이다(재지적: "글래스가 헬멧에 안 가려진다 — 더
         뒤쪽으로 쏙 넣고 반투명(투명에 가깝게)으로"). 이 반구들은 **화면 원**이라
         (halfSphereFaces3), 가리개가 앞으로 나온 만큼 화면에서 아래로 내려가 껍데기
         테두리 밖으로 삐져나왔다 — 그것이 '안 가려짐'의 정체다. 앞뒤 어긋남을 ±0.26 →
         ±0.14로 좁히고 가리개를 0.88배(0.57 대 0.65)로 두면 가리개 원이 껍데기 원 안에
         들어앉는다. 거기에 glassy()로 불투명도를 내려 유리로 읽힌다.
         ★ 크기는 2/3다(요청: "헬멧(글래스포함) 크기 2/3로 줄이기") — 0.98 → 0.65.
         고스트는 머리가 작아 같은 결로 0.68 → 0.45(가리개 0.4). */
      ...tagKey(halfSphereFaces3(0, -0.14, 4.26, 0.65, "#eef1f3"), depthNow(0, -0.14) + 1.6),
      ...tagKey(glassy(halfSphereFaces3(0, 0.14, 4.22, 0.57, "#7dff5c")), depthNow(0, 0.14) + 1.6),   // 메딕 — 흰 껍데기 · 네온 초록 얼굴가리개(요청)
      ...
      ...tagKey(paintBase([
        // 하완이 상완보다 굵게(요청) — 마린과 같은 규칙, 매딕은 한 단 작다: 0.44 / 0.6.
        ...hornFaces(-1.32, 0.1, 3.7, -1.1, 0.7, 2.8, 0.44),
        ...hornFaces(-1.1, 0.7, 2.8, -0.8, 1.3, 3.1, 0.6),
        ...hornFaces(1.36, 0.1, 3.7, 1.18, 0.5, 2.85, 0.44),
        ...hornFaces(1.18, 0.5, 2.85, 1.22, 0.98, 3.1, 0.6),
  ...
  gunner: () => {
    return [
      ...
      /* 두 팔(재지적: 위치·굽힘) — 위팔은 어깨뽕 '아래'(z 3.7)에서 나와 앞-아래로
         ...
         오른손은 방아쇠 쪽. */
      /* 하완이 상완보다 굵다(요청: "마린 매딕도 상완보다 하완 두껍게") — 파이어뱃에
         이미 준 규칙을 같은 파워드 아머를 입은 셋에 맞춘다. 상완 0.64 → 0.56,
         하완 0.54 → 0.72. 어깨에서 가늘게 나와 팔뚝에서 확 부푸는 장갑 실루엣이다. */
      ...hornFaces(-1.45, 0.1, 3.7, -1.05, 0.8, 2.75, 0.56),
      ...hornFaces(-1.05, 0.8, 2.75, 0.3, 1.85, 3.25, 0.72),
      ...hornFaces(1.5, 0.1, 3.7, 1.25, 0.7, 2.8, 0.56),
      ...hornFaces(1.25, 0.7, 2.8, 0.75, 1, 3.25, 0.72),
  ...
  fbat: () => {
    ...
    return [
      ...
      /* 몸은 붉다(요청: "파이어뱃 몸통 전체 빨간색") — 다리·몸통을 붉은 장갑색으로
         ...
         옆면에 불꽃무늬 개인색 데칼" — 직전 "어깨보호구 개인색" 요청을 덮는다).
         통짜 개인색 반구 한 쌍은 몸에서 색 덩어리로만 튀고 파이어뱃다운 표시가
         아니었다. 이제 임자 색은 허리띠와 어깨의 불꽃 데칼 둘이 맡는다. */
      ...paintBase([
        // 다리를 또렷하게(지적: 다리가 없어 헷갈림) — 벌린 두 기둥 + 둥근 발.
        ...cylinderFaces3(-0.64, 0, 0.52, 2.3, 0.1),
        ...cylinderFaces3(0.64, 0, 0.52, 2.3, 0.1),
      ...
      // 어깨보호구 — 몸통과 같은 붉은 장갑(요청). 임자 색은 아래 불꽃 데칼이 맡는다.
      ...paintBase([
        ...domeFaces3(-1.5, -0.3, 0.95, 0.85, 3.6),
        ...domeFaces3(1.5, -0.3, 0.95, 0.85, 3.6),
      ], "#b83a2c"),
      /* 어깨 옆면 불꽃 데칼(요청) — 혀 셋이 위로 타오르는 무늬. 칠을 안 해야 임자
         ...
         원근·앞숙임을 몸과 똑같이 타고 어깨에 붙어 돈다.
         판을 |x| 2.0에 두는 것이 이 무늬의 자(재기): 어깨 반구(중심 |x| 1.5, 반지름
         0.95, 높이 0.85)가 그 x에서 갖는 단면은 |Δy| 0.81 · Δz 0.72짜리 타원이고,
         무늬 꼭짓점이 전부 그 안에 들어야 어깨 밖으로 삐져나오지 않는다. 판이 어깨
         중심보다 카메라 쪽으로 나온 만큼 모델 원근이 무늬를 키우므로(지적 아닌 재기:
         2.15·반폭 0.5로는 사선 요잉에서 어깨 밖으로 조금 나갔다) 여유를 넉넉히 둔다. 어깨가 뒤로
         돌면 판이 제풀에 얇아지지만, 마주보는 몫이 얼마 안 되면 아예 걷는다. */
      ...([-1, 1] as const).flatMap((m8): ShapeFace[] => {
        ...
        const d8 = polyPath3(FLAME.map(([u8, v8]) => [m8 * 2, -0.3 + u8, 3.66 + v8]));
        // 제 어깨 바로 위(반구 키는 depthNow + 0.85) — 이웃 부품과는 안 다툰다.
        return tagKey([[d8, 1] as ShapeFace], depthNow(m8 * 1.5, -0.3) + 0.9);
      }),
      ...
      /* 두 팔(요청: "일반적인 팔로 바꾸고 하완이 상완보다 두껍게 붉은색 고정",
         ...
         **드는 자세가 다르다**: 오른손이 뒤 손잡이를 쥐고 왼손이 앞을 받친다. 그래서
         ...
         팔이 만드는 삼각형이 만든다. 팔은 몸통 밖에서 나와야 어깨뽕(±1.5) 실루엣에
         안 묻힌다. */
      ...paintBase([
        // 오른팔 — 어깨 → 팔꿈치 → 뒤 손잡이.
        ...rodFaces(1.6, 0.05, 3.55, 1.5, 0.95, 2.7, 0.58),
        ...rodFaces(1.5, 0.95, 2.7, 1.0, 1.35, 3.0, 0.8),
        // 왼팔 — 어깨 → 팔꿈치 → 몸 앞을 건너가 포신 앞을 받친다.
        ...rodFaces(-1.6, 0.05, 3.55, -1.25, 1.05, 2.65, 0.58),
        ...rodFaces(-1.25, 1.05, 2.65, 0.35, 2.15, 3.0, 0.8),
      ], "#8f2b20"),
      ...
      /* 호스(요청: "호스는 포신에서 아래로 늘어져서 연료통에 이어지게") — 어깨를
         가로지르던 멜빵을 걷는다. 포신 뒤끝에서 아래로 축 처졌다가, 발치를 지나 등
         연료통 밑동으로 올라간다. 마디 셋이 그 늘어짐을 만든다. */
      ...paintBase([
