...
import { bodyFace, capFace, groundEllipse, sideFace, topFace, type ShapeFace } from "../../utils/shapeOblique";
...
const ZERG_MOUND =
  ...
  /* 치마는 몸통과 같은 시계 감김이어야 한다(지적: 본 기둥과 다리 사이 빈 공간 — 반대
     감김끼리 겹치면 nonzero 채움이 그 자리를 구멍으로 뚫는다. shapeOblique.ts 규칙). */
  "M5.4 5.7 Q8 5.3 10.6 5.7 Q10.9 10.4 14.2 12.6 Q8 13.8 1.8 12.6 Q5.1 10.4 5.4 5.7 Z"
  /* 반원통 다리(지적: 아직도 바닥이 둥글어 보임) — 바닥 변은 완전한 수평 직선(L)이고
     모서리만 아주 작게 굴린다. 어깨 곡선이 완만하면 바닥까지 흘러내려 둥글게 읽힌다 —
     갈퀴는 좁고 가파른 마루로만 솟는다. */
  /* 앞줄 세 다리 — 방사형이다(지적: 셋이 나란히 앞만 보고 있었다): 가운데는 시청자
     쪽으로 곧장, 좌·우는 대각선 바깥-아래로 뻗는다. 끝 단면은 제 방향과 직각. */
  + " M11.3 10.4"
  + " Q13 11.3 14.6 12.8"
  + " Q15 13.4 14.4 13.7 L13.7 14 Q13.1 14.2 12.9 13.6"
  + " Q12.1 12.8 11.2 12.4"
  + " Q10.6 12.2 10.2 12.4 Q9.8 12.1 9.5 12.4"
  + " Q9.3 13.6 9.1 14.5 Q9 15.1 8.4 15.1 L7.6 15.1 Q7 15.1 6.9 14.5 Q6.7 13.6 6.5 12.4"
  + " Q6.2 12.1 5.8 12.4 Q5.4 12.2 4.8 12.4"
  + " Q3.9 12.8 3.1 13.6 Q2.9 14.2 2.3 14 L1.6 13.7 Q1 13.4 1.4 12.8"
  + " Q3 11.3 4.7 10.4"
  + " Q8 11.5 11.3 10.4 Z"
  /* 뒷줄 두 다리 — 몸통 옆구리에서 위-밖으로 뻗는 둥근 갈래(뒤로 뻗어 끝이 살짝 높다). */
  + " M12.8 10.6 Q14.8 9.4 15.9 10.2 Q16.4 10.9 15.6 11.5 Q14.2 12.3 12.9 11.8 Z"
  + " M3.2 10.6 Q1.2 9.4 0.1 10.2 Q-0.4 10.9 0.4 11.5 Q1.8 12.3 3.1 11.8 Z";
...
const ZERG_CAP_RF = "M12.95 13.6 Q13.3 12.75 14 12.95 Q14.6 13.2 14.35 13.72 Z";
const ZERG_CAP_CF = "M6.95 15.05 A1.08 1.02 0 0 1 9.05 15.05 Z";
const ZERG_CAP_LF = "M3.05 13.6 Q2.7 12.75 2 12.95 Q1.4 13.2 1.65 13.72 Z";
...
const SHAPE_FACES: Record<string, ShapeFace[]> = {
  ...
  lair: [
    [`${ZERG_MOUND}`
      + " M1.2 11.8 Q0.2 6.5 3 3.2 Q4.6 1.6 6 2.4 Q4.8 2.6 4 4.2 Q2.8 6.8 3.4 11.4 Z"
      + " M14.8 11.8 Q15.8 6.5 13 3.2 Q11.4 1.6 10 2.4 Q11.2 2.6 12 4.2 Q13.2 6.8 12.6 11.4 Z", 1],
    sideFace(ZERG_SHADE, 0.22),
    capFace(ZERG_CAPS_ALL),
    sideFace(ZERG_COLLAR, 0.35),
    topFace("M4.6 2 L6 2.4 L4.4 3.4 Z M11.4 2 L10 2.4 L11.6 3.4 Z", 0.5),
    topFace(ZERG_TOP),
  ],
  ...
  hive: [
    [`${ZERG_MOUND}`
      + " M1 11.8 Q-0.2 5.6 2.6 2 Q4.2 0.4 5.6 1.2 Q4.4 1.6 3.6 3.4 Q2.4 6.6 3.2 11.4 Z"
      + " M6.6 10.6 Q6.2 4 8 0.6 Q9.8 4 9.4 10.6 Z"
      + " M15 11.8 Q16.2 5.6 13.4 2 Q11.8 0.4 10.4 1.2 Q11.6 1.6 12.4 3.4 Q13.6 6.6 12.8 11.4 Z"
      + " M2.6 14.6 Q1.4 15.4 0.8 14.6 Q1.6 14.4 2 13.8 Z"
      + " M13.4 14.6 Q14.6 15.4 15.2 14.6 Q14.4 14.4 14 13.8 Z", 1],
    sideFace(ZERG_SHADE, 0.22),
    capFace(ZERG_CAPS_ALL),
    sideFace(ZERG_COLLAR, 0.35),
    topFace("M4.2 0.8 L5.6 1.2 L4 2.4 Z M11.8 0.8 L10.4 1.2 L12 2.4 Z M7.3 1.4 Q8 0.2 8.7 1.4 L8 2.6 Z", 0.5),
    topFace(ZERG_TOP),
  ],
