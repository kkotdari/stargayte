...
export const SHAPE_BUILDERS: Record<string, () => ShapeFace[]> = {
  ...
  gspire: () => {
    ...
    out.push(...hornFaces(2, -0.4, 6.6, 2.9, -0.9, 9.8, 0.95));
    /* 꼭대기(재지적: 너무 기계 같음 — 생명체답게) — 반듯한 고리·방사 골 대신 살덩이
       엽들이 아가리를 두르는 유기체 분화구. */
    const [cx2, cy2] = project(0, 0, 8.5);
    out.push(...domeFaces3(-1.2, -0.6, 1.15, 0.95, 7.9));
    out.push(...domeFaces3(1.2, -0.6, 1.1, 0.9, 7.9));
    out.push(...domeFaces3(-0.8, 0.7, 1, 0.85, 7.9));
    out.push(...domeFaces3(0.9, 0.7, 1, 0.85, 7.9));
    out.push(...domeFaces3(0, -1.2, 1, 0.85, 7.9));
    out.push(capFace(groundEllipse(cx2, cy2 - 0.15, 0.8, 0.5), 0.5));
    // 줄기 옆 불룩 혹 — 매끈함을 깬다.
    out.push(...domeFaces3(-1.8, 0.9, 1, 0.85, 3.2));
    out.push(...domeFaces3(1.9, 0.6, 0.9, 0.75, 4.4));
    return out;
  },
  ...
  /* 리버(다시) — 몸을 가로지르는 둥근 갑각 띠들이 겹겹이 얹힌 등딱지: 가운데가 불룩한
     혹등이고 앞은 낮은 머리와 집게. */
  reaver: () => [
    ...tubeFaces(-1.7, -2.6, 1.7, -2.6, 0.95, 2.5),
    ...tubeFaces(-1.9, -1.4, 1.9, -1.4, 1.05, 2.95),
    ...tubeFaces(-2, -0.1, 2, -0.1, 1.1, 3.2),
    ...tubeFaces(-1.9, 1.2, 1.9, 1.2, 1.05, 2.95),
    ...tubeFaces(-1.7, 2.4, 1.7, 2.4, 0.95, 2.5),
    // 낮은 머리 + 집게 한 쌍.
    ...domeFaces3(0, 3.5, 1, 0.8),
    ...hornFaces(0.5, 4.1, 0.6, 0.85, 4.8, 0.15, 0.35),
    ...hornFaces(-0.5, 4.1, 0.6, -0.85, 4.8, 0.15, 0.35),
  ],
  /* 레이스(실물 참고) — 홀쭉한 몸통과 콕핏, 뒤 양옆의 시가 나셀 둘, 아래로 젖힌
     날개판, 앞으로 내민 가는 쌍포. 미사일 포신은 몸 아래 매달림(유지). */
  wraith: () => {
    const hull = polyPath3([
      [0, 3.8, 6.1], [0.85, 0.8, 6.2], [0.65, -2.2, 6.1], [-0.65, -2.2, 6.1], [-0.85, 0.8, 6.2],
    ]);
    const [cx2, cy2] = project(0, 1.3, 6.25);
    const nose = (tx: number): ShapeFace => {
      const [px2, py2] = project(tx, -0.6, 5.6);
      return bodyFace(groundEllipse(px2, py2 - 0.25, 0.52, 0.42));
    };
    return [
      // 뒤 양옆 시가 나셀(앞끝 뭉뚝).
      ...tubeFaces(-1.9, -2.9, -1.9, -0.8, 0.55, 5.6),
      nose(-1.9),
      ...tubeFaces(1.9, -2.9, 1.9, -0.8, 0.55, 5.6),
      nose(1.9),
      // 아래로 젖힌 날개판.
      bodyFace(polyPath3([[-0.8, 0.4, 6], [-2.9, -1.4, 5.2], [-2.4, -2, 5.3], [-0.8, -0.8, 6]])),
      sideFace(polyPath3([[-0.8, 0.4, 6], [-2.9, -1.4, 5.2], [-2.4, -2, 5.3], [-0.8, -0.8, 6]]), 0.16),
      bodyFace(polyPath3([[0.8, 0.4, 6], [2.9, -1.4, 5.2], [2.4, -2, 5.3], [0.8, -0.8, 6]])),
      topFace(polyPath3([[0.8, 0.4, 6], [2.9, -1.4, 5.2], [2.4, -2, 5.3], [0.8, -0.8, 6]]), 0.14),
      // 몸통 + 콕핏 + 앞 가는 쌍포.
      bodyFace(hull), topFace(hull, 0.16),
      capFace(groundEllipse(cx2, cy2, 0.42, 0.34), 0.4),
      ...hornFaces(0.45, 3.2, 6, 0.65, 5.2, 5.9, 0.22),
      ...hornFaces(-0.45, 3.2, 6, -0.65, 5.2, 5.9, 0.22),
      // 아래 매달린 미사일 포신.
      ...hornFaces(0, -1.5, 5.8, 0, -1.7, 3.6, 0.26),
      ...tubeFaces(0, -2.6, 0, -1, 0.4, 3.1),
    ];
  },
  /* 배틀크루저(실물 참고) — 넓은 화살촉 몸통, 뒤 엔진 통 둘, 몸에서 대각으로 뻗는
     긴 붐 팔 한 쌍과 그 끝의 포드, 등뼈 마디와 함교. */
  bc: () => {
    const hull = polyPath3([
  ...
  observer: () => [
    ...
    ...hornFaces(0.5, -0.3, 6.5, 1, -0.9, 7.1, 0.3),
    ...hornFaces(-0.5, -0.3, 6.5, -1, -0.9, 7.1, 0.3),
    ...hornFaces(0.5, 0.4, 5.9, 1, 0.85, 5.2, 0.3),
    ...hornFaces(-0.5, 0.4, 5.9, -1, 0.85, 5.2, 0.3),
  ],
  ...
  zling: () => [
    // 긴 꼬리(지적) — 뒤로 땅을 끌며 등에 가시 둘.
    ...hornFaces(0, -1.2, 3.7, 0, -3.2, 3.4, 0.7),
    ...hornFaces(0, -3.2, 3.4, 0, -4.8, 3.2, 0.45),
    ...hornFaces(0, -2.2, 3.8, 0, -2.5, 4.7, 0.3),
    ...hornFaces(0, -3.4, 3.6, 0, -3.7, 4.4, 0.28),
    // 몸통은 더 낮게(지적) — 웅크린 낮은 통.
    ...cylinderFaces3(0, -0.6, 1.1, 1.3, 3.4),
    ...domeFaces3(0, 0.3, 0.85, 0.65, 4.6),
    ...claw3(1, 0.8, 3.7),
    ...claw3(-1, 0.8, 3.7),
  ],
  hydra: () => [
    // 다리는 없고 꼬리가 길다(지적) — 뒤로 땅을 끄는 두 마디 꼬리.
    ...hornFaces(0, -1.4, 4, 0, -3.4, 3.6, 0.9),
    ...hornFaces(0, -3.4, 3.6, 0, -5.2, 3.3, 0.6),
    ...
    // 머리 뒷장식은 구두주걱(정정) — 머리에 딱 붙어 뒤로 갈수록 넓어졌다 둥글게 끝나는
    // 살짝 오목한 주걱 판.
    bodyFace(polyPath3([
      [0.28, 0.1, 7.6], [0.5, -1.2, 8], [0.6, -2.6, 8.4], [0.35, -3.7, 8.6], [0, -4.1, 8.65],
      [-0.35, -3.7, 8.6], [-0.6, -2.6, 8.4], [-0.5, -1.2, 8], [-0.28, 0.1, 7.6],
    ])),
    topFace(polyPath3([
      [0.28, 0.1, 7.6], [0.5, -1.2, 8], [0.6, -2.6, 8.4], [0.35, -3.7, 8.6], [0, -4.1, 8.65],
      [-0.35, -3.7, 8.6], [-0.6, -2.6, 8.4], [-0.5, -1.2, 8], [-0.28, 0.1, 7.6],
    ...
    capFace(polyPath3([
      [0.2, -1, 8], [0.24, -2.8, 8.42], [0, -3.6, 8.58], [-0.24, -2.8, 8.42], [-0.2, -1, 8],
    ]), 0.2),
    ...claw3(1, 1, 4.4),
    ...claw3(-1, 1, 4.4),
  ],
  ultra: () => [
    // 튼튼한 다리 넷(지적) — 뒤 두 다리 먼저, 몸, 앞 두 다리.
    ...hornFaces(-1.8, -1.8, 3.8, -2.7, -2.4, 0.5, 1.05),
    ...hornFaces(1.8, -1.8, 3.8, 2.7, -2.4, 0.5, 1.05),
    // 큰 덩치(정정: 머리·몸통 다 크게) — 불룩한 큰 몸 + 큰 머리 + 카이저 칼날.
    ...domeFaces3(0, -0.9, 3.2, 3, 3.4),
    ...domeFaces3(0, 1.6, 2, 1.6, 3.9),
    ...hornFaces(-2, 0.9, 3.8, -2.9, 1.5, 0.5, 1.05),
    ...hornFaces(2, 0.9, 3.8, 2.9, 1.5, 0.5, 1.05),
    ...claw3(1, 1.4, 4.4),
    ...claw3(-1, 1.4, 4.4),
  ],
  ...
  lurker: () => {
    ...
    // 머리 뒷장식은 구두주걱(정정) — 머리에 딱 붙어 뒤로 넓어졌다 둥글게 끝난다.
    out.push(bodyFace(polyPath3([
      [0.26, 1.6, 4.6], [0.48, 0.4, 4.9], [0.56, -0.8, 5.2], [0.32, -1.7, 5.35], [0, -2, 5.4],
      [-0.32, -1.7, 5.35], [-0.56, -0.8, 5.2], [-0.48, 0.4, 4.9], [-0.26, 1.6, 4.6],
    ])));
    out.push(topFace(polyPath3([
      [0.26, 1.6, 4.6], [0.48, 0.4, 4.9], [0.56, -0.8, 5.2], [0.32, -1.7, 5.35], [0, -2, 5.4],
      [-0.32, -1.7, 5.35], [-0.56, -0.8, 5.2], [-0.48, 0.4, 4.9], [-0.26, 1.6, 4.6],
    ...
    out.push(capFace(polyPath3([
      [0.18, 0.4, 4.92], [0.22, -0.9, 5.2], [0, -1.6, 5.33], [-0.22, -0.9, 5.2], [-0.18, 0.4, 4.92],
    ]), 0.2));
    return out;
  },
