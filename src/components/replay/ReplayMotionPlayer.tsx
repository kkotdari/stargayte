...
/* 뒷다리 둘은 뒤로 뻗어(지적: 앞·옆·뒤 방향이 다 달라야) 입구가 이쪽을 안 본다 — 캡 없음.
   해처리 = 앞 셋(정면 크게·좌우 비스듬), 레어·하이브 = 가시 없는 앞 좌·우 둘만. */
...
const SHAPE_FACES: Record<string, ShapeFace[]> = {
  ...
  hatchery: [bodyFace(ZERG_MOUND), sideFace(ZERG_SHADE, 0.22), capFace(ZERG_CAPS_ALL), topFace(ZERG_TOP)],
  /* 레어(정정) — 바닥 기둥 끝에서 위로 솟는 가시. 쪽마다 두 기둥 중 하나에만 나고,
     높이는 본(중심) 기둥보다 높다. */
  /* 레어(정정) — 가시는 다리 기둥 끝에서 위로, 다리가 여섯이니 쌍마다 하나씩 셋. 사선
     시점이라 정좌우 대칭이 아니다: 뒤-왼쪽은 작게, 앞-가운데는 낮게, 오른쪽(가까운 쪽)은
     가장 크게. 높이는 본 기둥보다 높다. */
  lair: [
    [`${ZERG_MOUND}`
      + " M0.2 11.2 Q0 7.6 1.1 4.8 Q1.9 8.2 2.3 11.6 Z"
      + " M7.2 14.8 Q7 9.8 8 6.2 Q8.9 10.2 8.8 15 Z"
      + " M15.8 11.2 Q16.1 7 14.9 3.4 Q14.1 8 13.7 11.6 Z", 1],
    sideFace(ZERG_SHADE, 0.22),
    capFace(ZERG_CAPS_UNSPIKED),
    topFace(ZERG_TOP),
  ],
  /* 하이브 — 본 건물보다 훨씬 긴 뿔 셋이 위로 솟고, 가시는 그 뿔에서 본 건물 쪽(안쪽)
     으로 난다(지적). */
  /* 하이브 — 레어와 같은 세 자리(사선 비대칭)에서 본 건물보다 훨씬 긴 뿔이 솟고, 가시는
     뿔에서 본 건물 쪽(안쪽)으로 난다(지적). */
  hive: [
    [`${ZERG_MOUND}`
      + " M0.1 11.2 Q-0.2 6.4 1 2.4 Q2 7.4 2.4 11.6 Z"
      + " M7.1 14.8 Q6.7 9 7.9 3.2 Q9 9.4 8.9 15 Z"
      + " M15.9 11.2 Q16.3 5.8 14.9 0.8 Q13.9 7 13.6 11.6 Z"
      + " M0.9 7.4 L3.4 8.8 L1.4 9.8 Z M15 6.6 L12.5 8.2 L14.7 9.2 Z", 1],
    sideFace(ZERG_SHADE, 0.22),
    capFace(ZERG_CAPS_UNSPIKED),
    topFace(ZERG_TOP),
  ],
