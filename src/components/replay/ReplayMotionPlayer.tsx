...
export const SHAPE_BUILDERS: Record<string, () => ShapeFace[]> = {
  ...
  cube: () => {
    ...
    out.push(...tagKey(boxFaces3(-2.1, -0.2, 2.6, 5.4, 4.2, 2.9), 12 + depthNow(-2.1, -0.2)));
    out.push(...tagKey(boxFaces3(2.1, -0.2, 2.6, 5.4, 4.2, 2.9), 12 + depthNow(2.1, -0.2)));
    // 얇은 판 셋 — 가운데가 가장 높고 길다.
    out.push(...tagKey(boxFaces3(-4.1, 0, 1.5, 6.6, 6.2, 2.4), 14 + depthNow(-4.1, 0)));
    out.push(...tagKey(boxFaces3(4.1, 0, 1.5, 6.6, 6.2, 2.4), 14 + depthNow(4.1, 0)));
    out.push(...tagKey(boxFaces3(0, 0, 1.9, 7.4, 7, 2.4), 16 + depthNow(0, 0)));
    /* 뒤쪽 환풍구(사진) — 좌우 판 뒤끝에 비스듬히 앉은 격자 상자. 살짝 기울여 뒤로
       빠지고, 앞면에 밝은 살 세 줄. */
    for (const sx9 of [-1, 1] as const) {
      const vx9 = sx9 * 4.1;
      out.push(...tagKey([
        ...boxFaces3(vx9, -3.5, 1.9, 1.7, 1.5, 6.6),
        ...Array.from({ length: 3 }, (_, i9) => topFace(polyPath3([
          [vx9 - 0.8, -4.3 + i9 * 0.55, 8.15], [vx9 + 0.8, -4.3 + i9 * 0.55, 8.15],
          [vx9 + 0.8, -4.05 + i9 * 0.55, 8.15], [vx9 - 0.8, -4.05 + i9 * 0.55, 8.15],
        ]), 0.3)),
      ], 18 + depthNow(vx9, -3.5)));
    }
    /* 가운데 판 위 긴 환풍구 — 앞뒤로 길게 누운 낮은 상자에 가로 살들. */
    out.push(...tagKey([
      ...boxFaces3(0, -0.4, 1.5, 5.6, 1.1, 9.4),
      ...Array.from({ length: 5 }, (_, i9) => topFace(polyPath3([
        [-0.62, -2.8 + i9 * 1.1, 10.55], [0.62, -2.8 + i9 * 1.1, 10.55],
        [0.62, -2.45 + i9 * 1.1, 10.55], [-0.62, -2.45 + i9 * 1.1, 10.55],
      ]), 0.3)),
    ], 20 + depthNow(0, -0.4)));
    // 좌우 판 어깨의 밝은 띠(기존 포인트 유지).
    out.push(topFace(polyPath3([[-4.85, 2.2, 8.65], [-3.35, 2.2, 8.65], [-3.35, 1.4, 8.65], [-4.85, 1.4, 8.65]]), 0.3));
    out.push(topFace(polyPath3([[3.35, 2.2, 8.65], [4.85, 2.2, 8.65], [4.85, 1.4, 8.65], [3.35, 1.4, 8.65]]), 0.3));
    return out;
  },
