...
export const SHAPE_BUILDERS: Record<string, () => ShapeFace[]> = {
  ...
  tanksiege: () => {
    ...
    return [
      ...
      ...hornFaces(-1.3, 1.1, 1.8, -2.6, 2.1, 0.5, 0.75),
      ...boxFaces3(-2.8, 2.3, 1.6, 1.3, 0.55),
      ...hornFaces(-3.3, 2.7, 0.55, -3.6, 3, 0, 0.35),
      ...hornFaces(1.3, 1.1, 1.8, 2.6, 2.1, 0.5, 0.75),
      ...boxFaces3(2.8, 2.3, 1.6, 1.3, 0.55),
      ...hornFaces(3.3, 2.7, 0.55, 3.6, 3, 0, 0.35),
      ...hornFaces(-1.3, -1.1, 1.8, -2.7, -2.1, 0.5, 0.75),
      ...boxFaces3(-2.9, -2.3, 1.6, 1.3, 0.55),
      ...hornFaces(1.3, -1.1, 1.8, 2.7, -2.1, 0.5, 0.75),
      ...boxFaces3(2.9, -2.3, 1.6, 1.3, 0.55),
  ...
  /* 배틀크루저(합성 확정 다듬기) — 붐 날개·미사일을 더 뒤로, 추진기는 컵라면 용기
     셋을 붙인 꼴(뒤로 벌어지는 절두 원뿔 + 벌어진 테와 어두운 속). */
  bc: () => {
    const missile = (m2: 1 | -1): ShapeFace[] => [
      ...tubeFaces(m2 * 4.8, -1.4, m2 * 4.8, 1.2, 0.55, 5.3),
      ...hornFaces(m2 * 4.8, 1.2, 5.55, m2 * 4.8, 2.6, 5.5, 0.52),
    ...
    return [
      ...
      ...cup(0.95, 5.6),
      // 붐 날개 + 거대 미사일 — 더 뒤로(지적).
      ...hornFaces(-1.4, -1.4, 6, -4.5, -0.6, 5.7, 0.4),
      ...missile(-1),
      ...hornFaces(1.4, -1.4, 6, 4.5, -0.6, 5.7, 0.4),
      ...missile(1),
      // 장도리 몸 — 몸통·가로 머리·갈라진 두 발·함교.
  ...
  corsair: () => {
    ...
    return [
      // 몸은 둥글둥글하지 않게(지적) — 각진 방패 판.
      bodyFace(polyPath3([
        [0, 2.7, 5.9], [2.1, 1.3, 5.95], [2.6, -0.9, 5.85], [1.2, -2.2, 5.8],
        [-1.2, -2.2, 5.8], [-2.6, -0.9, 5.85], [-2.1, 1.3, 5.95],
      ])),
      topFace(polyPath3([[0, 2.7, 5.9], [-2.1, 1.3, 5.95], [-1.4, -0.4, 5.9], [0, 0.4, 5.92]]), 0.18),
      sideFace(polyPath3([[0, 2.7, 5.9], [2.1, 1.3, 5.95], [2.6, -0.9, 5.85], [1.1, -0.2, 5.9]]), 0.18),
      // 앞으로 감아 도는 옆 팔.
      ...hornFaces(1.9, 0.5, 5.9, 2.7, 2.8, 5.55, 0.75),
      ...hornFaces(-1.9, 0.5, 5.9, -2.7, 2.8, 5.55, 0.75),
      // 콕핏 혹.
      ...domeFaces3(0, 0.6, 0.8, 0.65, 6.1),
      // 뒤 발광 엔진 둘.
  ...
  /* 드라군(정정) — 몸통 축소 + 무릎이 위로 꺾이는 거미형 네 다리. */
  goon: () => {
    const leg = (m2: 1 | -1, fy: number): ShapeFace[] => [
      ...hornFaces(m2 * 1.4, fy, 4.4, m2 * 2.9, fy * 1.35, 5.4, 0.7),
      ...hornFaces(m2 * 2.9, fy * 1.35, 5.4, m2 * 3.5, fy * 1.55, 0.4, 0.55),
    ];
    return [
      ...leg(-1, -1.5), ...leg(1, -1.5),
      ...domeFaces3(0, -0.2, 2.2, 2, 4),
      ...leg(-1, 1.3), ...leg(1, 1.3),
    ];
  },
  ...
  ultra: () => [
    // 다리는 코끼리처럼 원통(지적: 뾰족 금지) — 뒤 두 다리 먼저, 몸, 앞 두 다리.
