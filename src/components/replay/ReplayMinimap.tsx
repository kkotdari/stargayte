...
/* ── 아바타 밑 체력바(요청) ────────────────────────────────────────────────────
   ...
   길이와 색은 같은 수를 본다(지적: "체력바 길이랑 색이랑 안 맞는데?"). 한때 길이에만
   제곱근을 씌워 뒀는데 — 규모 차이가 후반에 몇 배로 벌어져 뒤진 쪽이 실선이 되는 걸 막으려
   한 것이다 — 그러면 절반쯤 뒤진 사람의 막대가 71% 길이인데 색만 노랑이라, 보는 사람이
   길이에서 읽은 값과 색에서 읽은 값이 어긋난다. 색 문턱을 눈에 보이는 percent로 말한
   이상(50% 노랑 · 20% 빨강) 길이도 그 percent 그대로여야 말이 맞는다.

   제곱근을 걷어 낸 자리는 아래 최소 길이(8%)가 받는다 — 크게 뒤진 사람의 막대가 아예
   사라지지는 않는다. */
function PowerBar({ power, peak }: { power: number; peak: number }) {
  const ratio = peak > 0 ? Math.max(0, Math.min(1, power / peak)) : 0;
  /* 길이는 비율 그대로다 — 아래 색과 같은 수여야 한다(위 주석). 0에 가까워도 8%는 남겨
     둬서 '거의 지워진 사람'이 있다는 것 자체는 보이게 한다. */
  const len = Math.max(0.08, ratio);
