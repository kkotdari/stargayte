...
export default function GameResultStory({
  ...
  const actions = useMemo<{
    ...
    for (const n of idx) {
      ...
      for (const raw of [...actors, ...helpers, ...fightFoes]) {
        ...
        /* 흘려보낸 쪽의 표시는 자는 얼굴(😴)이다 — 자리는 아바타가 아니라 본진 가운데,
           그 사람이 세워 둔 병력 자리다(아래 markSpot). 아바타에 붙였을 때는 '사람이 존다'로
           읽혀서 뗐는데(요청), 같은 그림도 병력 위에 서면 '이 병력이 자고 있다'가 된다.
           잠 표시(💤)로 뒀다가 얼굴로 되돌린 값이다(요청) — 💤 한 장은 무엇이 자는지가
           안 보이고, 이 지도에서 다른 표시들도 다 얼굴·사물이라 혼자만 기호였다. */
     ...
     * 우선순위: 트로피 > 크게 무너짐 > 그 틈에 발전(🔥) > 적당히 당함 > 힘겨움(역부족)
     * > 도움받음(감동) > 잘 막아냄 > 도와줌(천사) > 공격자.
     * 흘려보낸 쪽은 이 표에 없다 — 얼굴이 아니라 세워 둔 병력에 잠 표시가 붙는다(요청). */
  ...
  // (삭제·요청) motionCaps — 재생 화면의 자막 표시를 걷었다.
