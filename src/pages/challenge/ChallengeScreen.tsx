...
export default function ChallengeScreen() {
  ...
  return (
    <div className="scr-screen scr-challenge-screen-v2">
      ...
      {/* 스크롤 스냅 여유 공간 — scroll-snap-align:start는 "오늘" 카드를 뷰포트 맨 위에
          맞추려면 그 뒤로 최소 한 화면 분량의 콘텐츠가 남아있어야 한다. "오늘" 뒤에 남은
          내용(그 이후 일정 + 맨 뒤로 옮겨진 미정)이 한 화면보다 짧으면 필요한 스크롤량이
          실제 최대 스크롤량을 넘어버려, 브라우저가 그냥 스크롤 맨 끝으로 잘라버려서
          "오늘"이 아니라 페이지 끝에 스냅이 걸리는 것처럼 보였다(실제로 지적받은 문제 —
          "타임라인에서 드래그해도... 끝에 자석처럼 당겨져서 끌려간다"). 목록 끝에 항상
          한 화면 분량의 여유를 둬서 그런 오버슈트가 구조적으로 생기지 않게 한다 —
          proximity라 이 여백으로 강제로 끌려가진 않는다. */}
      {!loading && <div className="scr-challenge-snap-spacer" aria-hidden="true" />}
