...
function MatchStack({ stack, memberOf, onDeleted, dateLabel, highlightMemberIds, highlightTerms }: {
  ...
  useLayoutEffect(() => {
    ...
    if (d <= 0 || reduced) {
      cancelRevealRef.current = () => {
      if (restList) restList.style.transform = ""; cleanupRail(); cancelRevealRef.current = null; };
      return;
    }
    ...
    railLocked = true;
    // 카드 기둥은 클립(rest-inner의 overflow:hidden) 밖 아래에서 시작해 위로 올라온다 —
    // 예전엔 여기에 transform을 안 걸고 "앞 카드가 덮어주니 안 보인다"에 기댔는데, 겹친
    // 장수가 많아 열린 높이가 앞 카드보다 커지면 그 아래로 삐져나온 카드들이 첫 프레임에
    // 통째로 드러났다(지적: "한 번에 카드가 다 렌더링돼서 뒤에 다 보여"). 클립 안에서
    // 직접 밀어 올리면 겹친 장수와 무관하게 항상 앞 카드 뒤에서 한 장씩 나온다.
    const restList = restListRef.current;
    const restH = restList ? restList.offsetHeight : 0;
    if (restList) restList.style.transform = `translateY(${restH}px)`;
    ...
    const start = () => {
      ...
      const a = driveStyle(slideDur, EASE_STACK, (p) => {
        ...
        if (restList) restList.style.transform = `translateY(${restH * (1 - p)}px)`;
      });
      void a.finished.then(() => {
        setTransform(belowLeaves, "");
        if (restList) restList.style.transform = "";
        railLocked = false;
  ...
  const closeStack = () => {
    ...
    anims.push(driveStyle(dur, EASE_STACK, (p) => {
      ...
      // 펼칠 때의 역순 — 카드 기둥이 클립 밖(아래)으로 도로 내려가며 한 장씩 사라진다.
      list.style.transform = `translateY(${restH * p}px)`;
    ...
    const finish = () => {
      ...
      // 커밋되면 rest가 0fr로 접히므로 인라인 transform은 걷어도 화면 변화가 없다.
