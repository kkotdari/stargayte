...
const ENTRY = `
import { SHAPE_BUILDERS, autoTier } from ${JSON.stringify(join(ROOT, "src/components/replay/ReplayMotionPlayer"))};
...
function inBrowser({ KINDS, PXQ, DPR, PAD, BUCKETS }) {
  ...
  for (const kind of KINDS) {
    ...
    if (ok) rows.push({ kind, faceN, ms, bytes, buckets: BUCKETS });
  }
  return rows;
}
...
console.log("kind          면수   굽기ms   한 방향 보관");
for (const r of rows.sort((a, b) => b.ms - a.ms)) {
  console.log(`${r.kind.padEnd(13)} ${String(r.faceN).padStart(4)}  ${f(r.ms).padStart(7)}   ${MB(r.bytes).padStart(6)}MB`);
}

/* 작업 집합 — 한 종류가 화면에 있으면 그 종류는 **방향 16 × 줌 갈래 × 임자 수**만큼
   갈린 판을 갖는다. 그 합이 예산을 넘으면 LRU가 매 프레임 쫓아내고 다시 굽는다. */
const perKindOneColor = rows.reduce((t, r) => t + r.bytes * BUCKETS, 0);
const bakeOneColor = rows.reduce((t, r) => t + r.ms * BUCKETS, 0);
console.log(`\n── 임자 수별 작업 집합(전 종류가 화면에 있고 16방향을 다 쓴다고 볼 때) `
  + "─".repeat(10));
console.log("임자   판 수      보관       예산 대비   전부 굽는 값");
for (let p = 1; p <= PLAYERS; p += 1) {
  const bytes = perKindOneColor * p;
  const plates = rows.length * BUCKETS * PXQ.length * p;
  const over = bytes / SPRITE_BYTES_MAX;
  const mark = over > 1 ? "  ← 예산 초과(LRU가 매 프레임 쫓아낸다)" : "";
  console.log(`${String(p).padStart(3)}  ${String(plates).padStart(7)}  ${MB(bytes).padStart(8)}MB`
    + `   ${(over * 100).toFixed(0).padStart(5)}%   ${f(bakeOneColor * p / 1000, 2).padStart(6)}s${mark}`);
}
